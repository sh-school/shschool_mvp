...
الهيكل الصحيح للتقييم في المدارس الحكومية القطرية:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
الفصل الأول  (40 درجة من أصل 100 سنوي)
  الباقة 1 — أعمال مستمرة   : 20 درجة  (50% من ال40)
  الباقة 4 — اختبار الفصل الأول : 20 درجة  (50% من ال40)

الفصل الثاني (60 درجة من أصل 100 سنوي)
  الباقة 1 — أعمال مستمرة   : 10 درجة  (≈17% من ال60)
  الباقة 3 — اختبار منتصف الفصل : 20 درجة  (≈33% من ال60)
  الباقة 4 — اختبار نهاية العام  : 30 درجة  (50%  من ال60)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
class SubjectClassSetup(models.Model):
    ...
    class_group   = models.ForeignKey(ClassGroup,  on_delete=models.CASCADE, related_name="subject_setups")
    teacher       = models.ForeignKey(CustomUser,  on_delete=models.CASCADE, related_name="subject_setups",
...
class AssessmentPackage(models.Model):
    ...
    # ─── الأوزان الصحيحة حسب سياسة وزارة التعليم القطرية ───
    #
    # الفصل الأول (semester_max=40):
    #   P1 أعمال مستمرة = 50%  → 50% × 40 = 20 درجة
    #   P2 اختبارات قصيرة = 0%  (غير مستخدمة في الفصل الأول)
    #   P3 نصفي = 0%             (غير مستخدم في الفصل الأول)
    #   P4 اختبار نهاية الفصل = 50% → 50% × 40 = 20 درجة
    #
    # الفصل الثاني (semester_max=60):
    #   P1 أعمال مستمرة = 17%  → 17% × 60 ≈ 10 درجة
    #   P2 اختبارات قصيرة = 0%  (مدمجة في P1)
    #   P3 اختبار منتصف = 33%  → 33% × 60 ≈ 20 درجة
    #   P4 اختبار نهاية العام = 50% → 50% × 60 = 30 درجة
    DEFAULT_WEIGHTS_S1 = {
        "P1": Decimal("50"),    # أعمال مستمرة → 20 من 40
        "P2": Decimal("0"),     # غير مستخدم في الفصل الأول
        "P3": Decimal("0"),     # غير مستخدم في الفصل الأول
        "P4": Decimal("50"),    # اختبار نهاية الفصل → 20 من 40
    }
    DEFAULT_WEIGHTS_S2 = {
        "P1": Decimal("16.67"), # أعمال مستمرة → ~10 من 60
        "P2": Decimal("0"),     # غير مستخدم (مدمج في P1)
        "P3": Decimal("33.33"), # اختبار منتصف الفصل الثاني → 20 من 60
        "P4": Decimal("50"),    # اختبار نهاية العام → 30 من 60
    }
    ...
    def __str__(self):
        return (f"{self.get_package_type_display()} | "
...
class Assessment(models.Model):
    ...
    id                = models.UUIDField(primary_key=True, default=_uuid, editable=False)
    package           = models.ForeignKey(AssessmentPackage, on_delete=models.CASCADE, related_name="assessments")
...
class StudentAssessmentGrade(models.Model):
    ...
    def status_label(self):
        if self.is_absent:  return "غائب"
        if self.is_excused: return "معذور"
        if self.grade is None: return "لم تُدخَل"
...
class StudentSubjectResult(models.Model):
    ...
    p1_score      = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True,
                                        verbose_name="درجة الباقة 1")
    p2_score      = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True,
                                        verbose_name="درجة الباقة 2")
    p3_score      = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True,
                                        verbose_name="درجة الباقة 3")
    p4_score      = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True,
                                        verbose_name="درجة الباقة 4")
    total         = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True,
                                        verbose_name="مجموع الفصل")
    semester_max  = models.DecimalField(max_digits=5, decimal_places=2, default=Decimal("40"),
    ...
    def __str__(self):
        return (f"{self.student.full_name} | {self.setup.subject.name_ar} | "
...
class AnnualSubjectResult(models.Model):
    ...
    setup         = models.ForeignKey(SubjectClassSetup, on_delete=models.CASCADE, related_name="annual_results")
    school        = models.ForeignKey(School, on_delete=models.CASCADE, related_name="annual_results")
    academic_year = models.CharField(max_length=9, default="2025-2026")

    # مجاميع الفصلين (من نتائجهما المحسوبة)
    s1_total      = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True,
                                        verbose_name="مجموع الفصل الأول (من 40)")
    s2_total      = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True,
                                        verbose_name="مجموع الفصل الثاني (من 60)")
    annual_total  = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True,
                                        verbose_name="المجموع السنوي (من 100)")
    pass_grade    = models.DecimalField(max_digits=5, decimal_places=2, default=Decimal("50"),
