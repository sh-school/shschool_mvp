...
class SubjectResultQuerySet(QuerySet):
    ...
    def class_average(self, class_group, semester: str = "S1") -> float | None:
        result = self.for_class(class_group).semester(semester).aggregate(avg=Avg("total"))
        return result["avg"]
...
class AnnualResultQuerySet(QuerySet):
    ...
    def counted(self) -> AnnualResultQuerySet:
        """ما يدخل العدَّ والمتوسّط — بلا «ليست مادة نجاح ورسوب» (`RESULT_STATUSES`)."""
        return self.filter(status__in=RESULT_STATUSES)
