...
from core.private_data import load_private_mapping
...
# ──────────────────────────────────────────────────
#: الأسماءُ الكاملةُ لموظّفين بيانٌ شخصيٌّ (PDPPL) والمستودعُ عامّ، فلا تُكتب في الشيفرة (REP-07b). يُقرأ الجدولُ من
#: ملفّ JSON خارج المستودع — `TEACHER_NAME_MAP_FILE` أو `data/teacher_name_map.json` (يتجاهله git) — على شكل
#: {"اسم الجدول": "اسم المنصّة الكامل"}. وغيابُ الملفّ جدولٌ فارغ: يُبلَّغ عن كلّ معلّمٍ لا يطابق اسمُه
#: اسمَه في المنصّة حرفيّاً بـ«معلم بدون مطابقة» ولا يسقط الاستيراد.
TEACHER_MAP_ENV = "TEACHER_NAME_MAP_FILE"
TEACHER_MAP_DEFAULT = "data/teacher_name_map.json"


def load_teacher_map():
    """اسمُ الجدول ← اسمُ المنصّة الكامل، من ملفٍّ خارج المستودع (فارغٌ إن غاب أو فسد)."""
    return load_private_mapping(TEACHER_MAP_ENV, TEACHER_MAP_DEFAULT)
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            help=(
                ...
                'مثال: --elective "معلم الكيمياء=الكيمياء"'
            ),
        )
        parser.add_argument(
            "--dry-run",
    ...
    def _prepare_data(self, teachers_data, teacher_id_map, classgroup_map, electives):
        ...
        for t in teachers_data:
            ...
            if not platform_name and (pdf_name in teacher_id_map or norm_pdf in teacher_id_map):
                # جدولُ الأسماء (`load_teacher_map`) للأسماء المختلفة وحدها. ومعلّمٌ اسمُه في
    ...
    def _relabel_parallel_by_teacher(self, schedule_rows, electives):
        ...
        فتُعلَن في السطر: `--elective "معلم الكيمياء=الكيمياء"`. والإعلان أصدق من
