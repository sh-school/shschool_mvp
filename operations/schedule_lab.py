"""مختبرُ جودة الجدول: خمسةٌ وعشرون مؤشراً تقيس المعلّمَ والشعبةَ والمادّةَ والموارد.
...
def metric_score(key: str, value) -> float | None:
    """درجةٌ من 0 إلى 100 تُشتقّ من المؤشر ليُقاس الجميعُ بمقياسٍ واحد.

    التحويلُ رتيبٌ: الأفضلُ أعلى دائماً، والقيمةُ الأصليّةُ تبقى بجانبه.

    وكان مكتوباً هنا أنّه «لا يدخل في أيّ حكمٍ أو توليد» — وذلك لم يعد صحيحاً
    منذ صار `grid_lab_score` يقيس محاولاتِ التوليد: هذه الدالّةُ تُستدعى لكلّ
    مؤشّرٍ في كلّ محاولة، ومجموعُها يفاضل بين الجداول ويقود المحسّن. فتعليقٌ
    يصف نيّةً انقضت أخطرُ من لا تعليق.
    """
...
def overall_score(metrics: dict) -> float:
    """درجةٌ واحدةٌ من 100: متوسّطُ درجات المؤشّرات — لا متوسّطُ متوسّطاتِ المجموعات.

    وكان متوسّطَ المجموعات، فأعطى كلَّ مجموعةٍ سُدسَ الدرجة مهما كان عددُ
    مؤشّراتها. والأعدادُ متفاوتة: تسعةٌ ليوم المعلّم واثنان للشعبة. فكان
    «الرياضياتُ في السادسة والسابعة» يزن أربعةَ أضعافٍ ونصفاً ما يزنه «أطولُ
    تتابع» — وزناً عَرَضيّاً لم يقرّره أحد، مصدرُه عددُ المؤشّرات في الحقيبة.

    وليس هذا شأنَ عرضٍ وحدَه: `grid_lab_score` يقيس به كلَّ محاولةِ توليدٍ
    فيدخل مفتاحَ المفاضلة، ويقوده `scheduler_improve` في كلّ نقلةٍ يقبلها.
    فالوزنُ العَرَضيُّ كان يقود الجدولَ لا يصفه.

    والتسويةُ بين المؤشّرات ليست حكماً بتساوي أهمّيّتها، بل رفضُ ترجيحٍ لم
    يُقصَد. وإن أرادت الإدارةُ ترجيحاً فليكن معلَناً في مكانه.

    وتبقى `section_scores` كما هي للرادار: هناك المطلوبُ متوسّطُ الحقيبة.
    """
    scores = [
        score
        for key in CATALOG
        if (score := metric_score(key, (metrics or {}).get(key, {}).get("value"))) is not None
    ]
