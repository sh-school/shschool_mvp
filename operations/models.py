...
class StaffEvaluation(models.Model):
    ...
    **مُهمَل (ADR-0002، 2026-09-19): لا قارئَ ولا كاتب.** حلّ محلَّه `quality.EmployeeEvaluation`
    (مستوياتُ المادة 16 الخمسة والاستماراتُ الوزاريّة). صفرٌ في الإنتاج فلا بياناتَ تُنقل. يبقى
    النموذجُ قائماً لإصدارٍ واحد لأنّ الحذفَ على خطوتين (CLAUDE.md، «توسيعٌ ثمّ تقليص»)؛ وحارسُ
    `tests/test_staff_evaluation_unused.py` يمنع أيَّ استعمالٍ جديد قبل هجرة الحذف.
    """
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                condition=models.Q(
                    professional_development__gte=1, professional_development__lte=5
                ),
