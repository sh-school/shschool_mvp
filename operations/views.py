...
from core.models import StudentEnrollment, ClassGroup
...
def attendance_view(request, session_id):
    ...
    existing = {
        att.student_id: att
        for att in StudentAttendance.objects.filter(session=session)
    }
...
def mark_single(request, session_id):
    ...
    return render(request, "teacher/partials/student_row.html", {
        "student":    student,
        "attendance": att,
        "status":     att.status,
        "session":    session,
...
def complete_session(request, session_id):
    ...
    messages.success(request, f"تم إنهاء الحصة بنجاح. الحضور: {session.present_count}/{session.attendance_count}")
...
def daily_report(request):
    ...
    ).select_related("student", "session__class_group").order_by(
        "session__class_group__grade", "student__full_name"
    )

    sessions = Session.objects.filter(school=school, date=report_date)
    from django.db.models import Count
    summary = StudentAttendance.objects.filter(
        school=school, session__date=report_date
...
# ══════════════════════════════════════════════════════
from .models import ScheduleSlot, TeacherAbsence, SubstituteAssignment
...
def teacher_absence_list(request):
    ...
    absences = TeacherAbsence.objects.filter(
        school=school, date=abs_date
    ).select_related("teacher", "reported_by").prefetch_related("assignments__substitute")
...
def register_teacher_absence(request):
    ...
    if request.method == "POST":
        from datetime import date as dclass
        teacher   = get_object_or_404(CustomUser, id=request.POST["teacher"], memberships__school=school)
...
def absence_detail(request, absence_id):
    ...
    slots   = ScheduleSlot.objects.filter(
        school=school, teacher=absence.teacher,
        day_of_week=our_day, is_active=True
    ).select_related("class_group", "subject")

    # للكل حصة: هل يوجد بديل؟
    assignments = {a.slot_id: a for a in SubstituteAssignment.objects.filter(absence=absence).select_related("substitute")}
...
def substitute_report(request):
    ...
    today      = timezone.now().date()
    date_from  = dclass.fromisoformat(request.GET.get("from", (today - timedelta(days=7)).isoformat()))
