...
استثناءً. وهذا يفترق عن `teacher.run_breaches` القائم الذي يعدّ حصصَ اليوم فيرى المزدوجةَ تتابعاً.
...
def _second_halves(lab: ScheduleLab) -> dict[tuple[str, int], set[int]]:
    ...
    for (tid, day, _class, _subject), found in periods.items():
        halves[(tid, day)].update(sorted(found)[1::2])
    return halves


def _longest_task_run(lab: ScheduleLab, tid: str, day: int, halves: set[int]) -> int:
    """أطولُ تتابعٍ متّصلٍ بالساعة لمهامّ معلّمٍ في يومه — المزدوجةُ مهمّةٌ واحدة."""
    best = run = 0
...
def exception_load(lab: ScheduleLab) -> dict:
    ...
    halves = _second_halves(lab)
    teacher_days: dict[str, int] = defaultdict(int)
    for tid, days in lab.by_teacher_day.items():
        cap = lab.run_cap(tid)
        teacher_days[tid] = sum(
            1
            for day in days
            if _longest_task_run(lab, tid, day, halves.get((tid, day), set())) > cap
        )
