...
المهام:
    ...
    3. حارسُ العام الدراسيّ — إطفاءُ جداول الأعوام الماضية (يومياً)
...
@shared_task(name="operations.retire_past_year_slots")
def retire_past_year_slots_task():
    """يُطفئ حصص الجدول الباقية نشطةً من أعوامٍ مضت، في كلّ مدرسة.
    ...
    for school in School.objects.all().iterator(chunk_size=100):
        with school_rls_scope(school.id):
            total += ScheduleService.retire_past_year_slots(school)
