...
from .models import CustomUser, Profile, School, Role, Membership, ClassGroup, StudentEnrollment, ParentStudentLink
...
class CustomUserAdmin(UserAdmin):
    ...
    add_fieldsets = (
        (None, {"classes": ("wide",), "fields": ("national_id", "full_name", "password1", "password2")}),
    )
...
class ParentStudentLinkAdmin(admin.ModelAdmin):
    list_display  = ("student", "parent", "relationship", "school", "can_view_grades", "can_view_attendance")
    list_filter   = ("school", "relationship", "can_view_grades", "can_view_attendance")
    search_fields = ("parent__full_name", "parent__national_id", "student__full_name", "student__national_id")
...
class AuditLogAdmin(admin.ModelAdmin):
    list_display  = ("timestamp", "user", "action", "model_name", "object_repr", "ip_address", "school")
    list_filter   = ("action", "model_name", "school")
    search_fields = ("user__full_name", "object_repr", "ip_address")
    readonly_fields = ("id", "timestamp", "user", "action", "model_name",
...
class ConsentRecordAdmin(admin.ModelAdmin):
    list_display  = ("parent", "student", "data_type", "is_given", "method", "given_at", "withdrawn_at")
