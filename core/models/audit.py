...
class ErasureRequest(models.Model):
    ...
    student       = models.ForeignKey(CustomUser, on_delete=models.SET_NULL,
                                      null=True, related_name='erasure_requests')
    requested_by  = models.ForeignKey(CustomUser, on_delete=models.SET_NULL,
                                      null=True, related_name='erasure_filed')
    ...
    reviewed_by   = models.ForeignKey(CustomUser, on_delete=models.SET_NULL,
                                      null=True, blank=True, related_name='erasure_reviewed')
    review_note   = models.TextField(blank=True, verbose_name='ملاحظات المراجع')
    created_at    = models.DateTimeField(auto_now_add=True)
    reviewed_at   = models.DateTimeField(null=True, blank=True)
    completed_at  = models.DateTimeField(null=True, blank=True)
    anonymized_id = models.CharField(max_length=20, blank=True,
                                     verbose_name='المعرّف المجهّل',
                                     help_text='مثل: ERASED-0001')
    summary       = models.JSONField(null=True, blank=True,
                                     verbose_name='ملخص البيانات المحذوفة')
