...
class UserQuerySet(models.QuerySet):
    ...
    def search(self, query: str) -> "UserQuerySet":
        ...
        q = query.strip()[:100]
        vector = (
            SearchVector("full_name", weight="A", config="arabic")
            + SearchVector("national_id", weight="B")
        )
    ...
    def search_fuzzy(self, query: str, threshold: float = 0.2) -> "UserQuerySet":
        ...
        return (
            ...
            .filter(
                Q(sim_name__gte=threshold)
                | Q(sim_national__gte=threshold)
            )
            .order_by("-sim_name")
        )
    ...
    def with_enrollment(self) -> "UserQuerySet":
        ...
        return self.prefetch_related(
            Prefetch(
                ...
                queryset=__import__(
                    "operations.models", fromlist=["StudentEnrollment"]  # type: ignore[attr-defined]
