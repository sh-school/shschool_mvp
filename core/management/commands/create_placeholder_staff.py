"""
يُنشئ سجلّاً مؤقّتاً لمعلّمٍ جديدٍ باسمه المختصر حتى تُستكمل بياناته.

    python manage.py create_placeholder_staff --name "جمال صالح" --apply

المعلّم الجديد يظهر في جدول الحصص قبل أن يُدخل في المنصّة، فتسقط حصصه
كلّها عند الاستيراد وتبقى شعبٌ بلا مادّة. وهذا السجلّ يسدّ الفجوة حتى
تُدخل شؤون الموظفين بياناته الكاملة.
...
class Command(BaseCommand):
    help = "يُنشئ سجلّاً مؤقّتاً لمعلّمٍ جديد باسمه المختصر"
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            required=True,
            metavar="اسم المعلّم",
            help='يُكرَّر لكل معلّم: --name "جمال صالح" --name "علي الطيطي"',
        )
