...
def map_role(job_title_norm: str) -> str:
    ...
    if "مدير المدرسه" in t or "مدير المدرسة" in t: return "principal"
    if "النائب الاداري" in t:                        return "vice_admin"
    if "نائب المدير" in t or "الاكاديمي" in t:      return "vice_academic"
    if t.startswith("منسق"):                          return "coordinator"
    if t.startswith("معلم"):                          return "teacher"
    if "اخصائي" in t or "مرشد" in t:                return "specialist"
    return "admin"
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--reset", action="store_true",
                            help="حذف البيانات السابقة وإعادة الحقن")
        parser.add_argument("--step",  default="all",
                            choices=["all","school","staff","students","links",
                                     "subjects","schedule","attendance",
                                     "grades","quality","notifications"],
    ...
    def _step_school(self):
        from core.models import School, Role
        ...
        role_objs = {}
        for rname in ["principal","vice_admin","vice_academic","coordinator",
                      "teacher","specialist","admin","student","parent"]:
    ...
    def _step_staff(self, school, role_objs):
        ...
        for row in staff_rows:
            ...
            if c:
                ...
                user.full_name = name; user.email = email or user.email
                user.is_staff = True; user.save(update_fields=["full_name","email","is_staff"])
            ...
            Membership.objects.get_or_create(
                user=user, school=school, role=role_objs[role_name],
    ...
    def _step_students(self, school, role_objs):
        from core.models import CustomUser, Membership, Profile, ClassGroup, StudentEnrollment
        ...
        stu_map = {}; par_map = {}; stu_c = 0; par_c = 0; enr_c = 0
        ...
        for row in student_rows:
            ...
            stu, c = CustomUser.objects.get_or_create(
                national_id=nat_id,
                defaults={"full_name": name_ar, "is_staff": False}
            )
            if c:
                stu.set_password("student@2026"); stu.save(); stu_c += 1
            else:
                stu.full_name = name_ar; stu.save(update_fields=["full_name"])

            Profile.objects.get_or_create(user=stu)
            Membership.objects.get_or_create(
                user=stu, school=school, role=role_objs["student"],
                defaults={"is_active": True}
            )
            ...
            if par_nat and par_name:
                par, pc = CustomUser.objects.get_or_create(
                    national_id=par_nat,
                    defaults={"full_name": par_name,
                              "phone": par_phone,
                              "email": par_email if par_email not in ("-","") else ""}
                )
                if pc:
                    par.set_password("parent@2026"); par.save(); par_c += 1
                else:
                    par.full_name = par_name; par.save(update_fields=["full_name"])

                Profile.objects.get_or_create(user=par)
                Membership.objects.get_or_create(
                    user=par, school=school, role=role_objs["parent"],
                    defaults={"is_active": True}
                )
                par_map[par_nat] = par
    ...
    def _load_maps(self, school):
        from core.models import CustomUser, ClassGroup, StudentEnrollment
        class_map = {}
        GRADE_MAP = {"7":"G7","8":"G8","9":"G9","10":"G10","11":"G11","12":"G12"}
    ...
    def _step_parent_links(self, school, student_map, parent_map):
        ...
        REL_MAP = {"أب":"father","أم":"mother","الأب":"father","الأم":"mother",
                   "وصي":"guardian","ولي":"guardian"}
    ...
    def _step_schedule(self, school, subject_map, class_map, role_objs):
        ...
        teachers = list(CustomUser.objects.filter(
    ...
    def _step_attendance(self, school, class_map):
        from operations.models import Session, StudentAttendance, ScheduleSlot
        ...
        for day_offset in range(14, 0, -1):
            ...
            for slot in slots:
                session, s_created = Session.objects.get_or_create(
                    school=school, teacher=slot.teacher,
                    class_group=slot.class_group, date=d,
    ...
    def _step_grades(self, school, subject_map, class_map, role_objs):
        ...
        from core.models import CustomUser, StudentEnrollment, Membership
        ...
        for i, cg in enumerate(class_list):
            for j, subj in enumerate(subj_list[:6]):
                ...
                for semester, sem_max, weights in [
                    ...
                    ("S2", Decimal("60"), {"P1": Decimal("16.67"), "P3": Decimal("33.33"), "P4": Decimal("50")}),
                ]:
                    for ptype, weight in weights.items():
                        ...
                        title_map = {
                            ...
                            "P4": "اختبار نهاية الفصل" if semester=="S1" else "اختبار نهاية العام",
                        }
                        ...
                        for enr in students:
                            ...
                            grade_val = Decimal(str(round(
                                random.gauss(72, 15), 1
    ...
    def _step_quality(self, school, role_objs):
        from quality.models import (
            ...
            OperationalProcedure, QualityCommitteeMember, ExecutorMapping
        )
        ...
        for row in plan_rows:
            ...
            if d_name not in domain_map:
                domain, _ = OperationalDomain.objects.get_or_create(
                    school=school, name=d_name, academic_year=YEAR,
            ...
            if i_key not in indicator_map:
                indicator, _ = OperationalIndicator.objects.get_or_create(
                    target=target, number=i_no,
                    defaults={"text": i_text}
                )
            ...
            _, c = OperationalProcedure.objects.get_or_create(
                school=school, indicator=indicator, number=p_no,
                defaults={
                    "text":                     p_text,
                    "executor_norm":            executor,
                    "date_range":               date_r,
                    "status":                   status if status in ("In Progress","Completed","Not Started","Cancelled") else "In Progress",
        ...
        for row in quality_rows:
            ...
            _, c = QualityCommitteeMember.objects.get_or_create(
                school=school, job_title=title, academic_year=YEAR,
                defaults={
                    "responsibility": resp if resp in ["رئيس اللجنة","نائب رئيس اللجنة","مقرر","عضو"] else "عضو",
                    "domain":         domain_obj,
                    "is_active":      True,
                }
            )
            if c: committee_created += 1
        ...
        try:
            ...
            for row in exec_rows:
                ...
                user = CustomUser.objects.filter(
                    ...
                    memberships__role__name__in=["coordinator","teacher","specialist","admin","vice_admin","vice_academic","principal"]
                ).filter(
                    memberships__user__full_name__icontains=keyword
                ).first()
                ...
                mapping, c = ExecutorMapping.objects.get_or_create(
                    school=school, executor_norm=exec_norm, academic_year=YEAR,
                    defaults={"user": user}
                )
                if not c and user and not mapping.user:
                    mapping.user = user; mapping.save(update_fields=["user"])
    ...
    def _print_summary(self, school):
        from core.models import CustomUser, ClassGroup, StudentEnrollment, Membership
        ...
        staff_count = Membership.objects.filter(
            ...
            role__name__in=["principal","vice_admin","vice_academic",
        ...
        principal = Membership.objects.filter(
            school=school, role__name="principal", is_active=True
