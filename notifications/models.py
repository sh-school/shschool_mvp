...
class NotificationDelivery(models.Model):
    ...
    # ── [B4-4] ميزانية المحاولات — دائمة لا في رسالة ──────────────────
    #
    # رُفض هذا العمود في B4-A بحجّة أن المحاولات صفوفٌ في `NotificationLog`
    # فيكون عدُّها نسخةً ثانية تنحرف. والحجّة كانت صحيحة يومها لأن الميزانية
    # كانت تعيش في `request.retries` داخل رسالة Celery، ولم يكن هناك من يُعيد
    # إنشاء الرسالة.
    #
    # ثم صار للمُصالِح أن يُعيد طبر `retry_wait` — والمهمّة الجديدة تبدأ من
    # `request.retries = 0`. فلو تكرّر فشلُ نشر الإعادة بعد كتابة `retry_wait`،
    # أعاد المُصالِح دورةً كاملة كلّ مرّة: لا حدّ فعليّ للمحاولات إطلاقاً.
    #
    # والعدّ من `NotificationLog` لا يصلح بديلاً: صفوفه تُكتب **بعد** نداء
    # المزوّد، فعاملٌ مات قبل الكتابة لا يترك أثراً — والمحاولة التي تُفقد من
    # العدّ هي بالضبط المحاولة الخطيرة.
    #
    # يزيده `claim_delivery` وحده وذرّياً: الاستحواذ **هو** المحاولة.
