...
class ExamSession(models.Model):
    ...
    school        = models.ForeignKey('core.School', on_delete=models.CASCADE, related_name='exam_sessions')
    ...
    created_by    = models.ForeignKey('core.CustomUser', null=True, on_delete=models.SET_NULL,
                                      related_name='created_exam_sessions')

    class Meta:
        verbose_name = 'دورة اختبار'; verbose_name_plural = 'دورات الاختبارات'
        ordering = ['-start_date']
...
class ExamRoom(models.Model):
    ...
    class Meta:
        verbose_name = 'قاعة اختبار'; verbose_name_plural = 'قاعات الاختبار'
        ordering = ['name']

    def __str__(self): return f"قاعة {self.name} — {self.session.name}"


class ExamSupervisor(models.Model):
    """تشكيل الكنترول — المحور 1"""
    ROLES = [
        ('head','رئيس الكنترول'),('deputy','نائب الرئيس'),
        ('secretary','أمين السر'),('print','مسؤول الطباعة'),
        ('safe','مسؤول الخزائن'),('delivery','مسؤول التسليم'),
        ('grading','مسؤول الرصد'),('audit','مسؤول التدقيق'),
        ('supervisor','مشرف قاعة'),('observer','مراقب'),
    ]
    id          = models.UUIDField(primary_key=True, default=_uuid, editable=False)
    session     = models.ForeignKey(ExamSession, on_delete=models.CASCADE, related_name='supervisors')
    room        = models.ForeignKey(ExamRoom, null=True, blank=True, on_delete=models.SET_NULL, related_name='supervisors')
    staff       = models.ForeignKey('core.CustomUser', on_delete=models.CASCADE, related_name='exam_roles')
    role        = models.CharField(max_length=15, choices=ROLES, default='supervisor')
    assigned_at = models.DateTimeField(auto_now_add=True)

    class Meta:
        verbose_name = 'مشرف كنترول'; verbose_name_plural = 'مشرفو الكنترول'
        unique_together = [('session','staff')]

    def __str__(self): return f"{self.staff.full_name} — {self.get_role_display()}"
...
class ExamSchedule(models.Model):
    ...
    class Meta:
        verbose_name = 'جدول اختبار'; verbose_name_plural = 'جداول الاختبارات'
        ordering = ['exam_date','start_time']

    def __str__(self): return f"{self.subject} — {self.grade_level} — {self.exam_date}"
...
class ExamIncident(models.Model):
    ...
    """
    TYPES    = [('cheating','غش مؤكد'),('misconduct','سوء سلوك'),
                ('medical','حالة طبية'),('technical','عطل تقني'),('other','أخرى')]
    ...
    room          = models.ForeignKey(ExamRoom, null=True, blank=True, on_delete=models.SET_NULL, related_name='incidents')
    student       = models.ForeignKey('core.CustomUser', null=True, blank=True,
                                       on_delete=models.SET_NULL, related_name='exam_incidents',
                                       verbose_name='الطالب المعني')
    reported_by   = models.ForeignKey('core.CustomUser', null=True, on_delete=models.SET_NULL,
    ...
    behavior_link = models.ForeignKey('behavior.BehaviorInfraction', null=True, blank=True,
                                       on_delete=models.SET_NULL, related_name='exam_incidents')

    class Meta:
        verbose_name = 'حادث اختبار'; verbose_name_plural = 'حوادث الاختبارات'
        ordering = ['-incident_time']

    def __str__(self): return f"{self.get_incident_type_display()} — {self.session.name}"
...
class ExamEnvelope(models.Model):
    ...
    """
    ACTIONS = [('received','استُلمت'),('opened','فُتحت'),('distributed','وُزِّعت'),('returned','أُعيدت')]
    ...
    witness     = models.ForeignKey('core.CustomUser', null=True, blank=True, on_delete=models.SET_NULL,
    ...
    class Meta:
        verbose_name = 'محضر مظروف'; verbose_name_plural = 'محاضر المظاريف'
        ordering = ['timestamp']
...
class ExamGradeSheet(models.Model):
    ...
    id           = models.UUIDField(primary_key=True, default=_uuid, editable=False)
    schedule     = models.ForeignKey(ExamSchedule, on_delete=models.CASCADE, related_name='grade_sheets')
    grader       = models.ForeignKey('core.CustomUser', null=True, on_delete=models.SET_NULL,
    ...
    class Meta:
        verbose_name = 'ورقة رصد'; verbose_name_plural = 'أوراق الرصد'

    def __str__(self): return f"رصد: {self.schedule} — {self.get_status_display()}"
