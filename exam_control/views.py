...
from .models import ExamSession, ExamRoom, ExamSupervisor, ExamSchedule, ExamIncident, ExamGradeSheet


def _can_access(user):
    return user.is_authenticated and (
        user.is_admin() or user.is_superuser or
        hasattr(user, 'role') and user.role and user.role.name in ('exam_head', 'warden', 'principal')
    )
...
def dashboard(request):
    ...
    sessions = ExamSession.objects.filter(school=school).annotate(
        incident_count=Count('incidents'),
        pending_sheets=Count('schedules__grade_sheets', filter=Q(schedules__grade_sheets__status='pending')),
    ).order_by('-start_date')
...
def session_create(request):
    ...
    return render(request, 'exam_control/session_form.html', {
...
def incidents(request, pk):
    ...
    school = request.user.get_school()
    session = get_object_or_404(ExamSession, pk=pk, school=school)
    qs = session.incidents.select_related('student', 'room', 'reported_by').order_by('-incident_time')
...
def incident_add(request, pk):
    ...
    if request.method == 'POST':
        ...
        incident = ExamIncident.objects.create(
            session=session, room=room, student=student,
...
def incident_pdf(request, pk):
    ...
    incident = get_object_or_404(ExamIncident, pk=pk,
                                  session__school=request.user.get_school())
    html_str = render_to_string('exam_control/pdf/incident_report.html', {
        'incident': incident,
...
def grade_sheets(request, pk):
    ...
    sheets = ExamGradeSheet.objects.filter(
        schedule__session=session
    ).select_related('schedule__room', 'grader').order_by('schedule__exam_date')
...
def session_report_pdf(request, pk):
    ...
    school = request.user.get_school()
    session = get_object_or_404(ExamSession, pk=pk, school=school)
    html_str = render_to_string('exam_control/pdf/session_report.html', {
        'session': session,
        'supervisors': session.supervisors.select_related('staff', 'room'),
        'schedules': session.schedules.select_related('room').order_by('exam_date'),
        'incidents': session.incidents.select_related('student', 'room').order_by('incident_time'),
        'sheets': ExamGradeSheet.objects.filter(schedule__session=session).select_related('schedule'),
        'generated_at': timezone.now(),
        'generated_by': request.user,
    })
