...
class KPIService:
    ...
    def compute(school, year: str = "2025-2026") -> dict:
        ...
        """
        from django.utils import timezone
        from core.models import StudentEnrollment, Membership, BookBorrowing
        from operations.models import Session, StudentAttendance, TeacherAbsence
        from behavior.models import BehaviorInfraction
        ...
        # ── 1: نسبة حضور الطلبة ──────────────────────────────────────
        sessions_m = Session.objects.filter(
        ...
        kpis["student_attendance_pct"] = {
            "label": "نسبة حضور الطلبة", "unit": "%", "frequency": "أسبوعي",
            "value": round(present / total_att * 100, 1) if total_att else 0,
            "target": 95, "warning": 90, "direction": "higher_better",
        }
        ...
        kpis["infractions_per_100"] = {
            "label": "مخالفات / 100 طالب", "unit": "", "frequency": "شهري",
            "value": round(infractions_m / total_students * 100, 2) if total_students else 0,
            "target": 3, "warning": 5, "direction": "lower_better",
        }
        ...
        kpis["grading_on_time_pct"] = {
            "label": "رصد الدرجات في الوقت", "unit": "%", "frequency": "فصلي",
            "value": grading_pct, "target": 98, "warning": 95, "direction": "higher_better",
        }

        # ── 4: أيام اختبار بلا حوادث ─────────────────────────────────
        try:
            from exam_control.models import ExamSession, ExamIncident
            exam_days     = set(ExamSession.objects.filter(school=school)
                                .values_list("start_date", flat=True))
            incident_days = set(ExamIncident.objects.filter(
                session__school=school, severity__gte=2
            ).values_list("incident_time__date", flat=True))
            clean_pct = (
        ...
        kpis["exam_clean_days_pct"] = {
            "label": "أيام اختبار بلا حوادث", "unit": "%", "frequency": "فصلي",
            "value": clean_pct, "target": 100, "warning": 99, "direction": "higher_better",
        }
        ...
        unexcused = TeacherAbsence.objects.filter(
            school=school, date__month=month, date__year=today.year, is_excused=False,
        ).count()
        kpis["teacher_unexcused_abs_pct"] = {
            "label": "غياب المعلمين غير المبرر", "unit": "%", "frequency": "شهري",
            "value": round(unexcused / teacher_days * 100, 2) if teacher_days else 0,
            "target": 1, "warning": 2, "direction": "lower_better",
        }

        # ── 6: حوادث النقل / 100,000 كم ──────────────────────────────
        kpis["transport_incidents_rate"] = {
            "label": "حوادث النقل / 100,000 كم", "unit": "", "frequency": "شهري",
            "value": 0, "target": 0, "warning": 0.1, "direction": "lower_better",
        ...
        kpis["library_borrows_per_student"] = {
            "label": "استعارة المكتبة / طالب", "unit": "", "frequency": "فصلي",
            "value": round(borrows / total_students, 2) if total_students else 0,
            "target": 2, "warning": 1, "direction": "higher_better",
        }

        # ── 8-10: مؤشرات يدوية ───────────────────────────────────────
        kpis["canteen_health_compliance_pct"] = {
            "label": "مطابقة المقصف للصحة", "unit": "%", "frequency": "فصلي",
            "value": None, "target": 95, "warning": 90, "direction": "higher_better",
            "note": "يُدخَل يدوياً من تقارير وزارة الصحة",
        }
        kpis["evacuation_plan_pct"] = {
            "label": "تنفيذ خطط الإخلاء", "unit": "%", "frequency": "فصلي",
            "value": None, "target": 100, "warning": 99, "direction": "higher_better",
            "note": "يُسجَّل بعد كل تدريب إخلاء",
        }
        kpis["professional_dev_hours"] = {
            "label": "ساعات التطوير المهني / معلم", "unit": "ساعة", "frequency": "فصلي",
            "value": None, "target": 10, "warning": 6, "direction": "higher_better",
