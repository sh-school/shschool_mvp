...
# عملٌ دُمج بلا بندٍ في الخارطة: (الرمز، المسار، العنوان، طلب الدمج[، صلةٌ ببند مالك])
NEW_ITEMS = [
    (
        "N-015",
        "frontend",
    ...
    (
        "N-019",
        "sec",
        "الإدارة: عزلُ المدارس في 14 نموذجاً يصل إليها المدير + حارسٌ بنيويّ",
        "#501",
        "يخصّ OWN-27.",
    ),
    (
        "N-020",
        "sec",
        "الإدارة: المديرُ يعدّل «المدرسة» ولا يضيفها ولا يحذفها",
        "#502",
        "يخصّ OWN-27 وOWN-29: الحارسُ فعّالٌ فوراً في الشيفرة، ومزامنةُ المجموعة في الإنتاج بإذن المالك.",
    ),
    (
        "N-021",
        "frontend",
        "الإدارة: فوترُ لوحة الإدارة مطابقٌ لفوتر المنصّة الجديد",
        "#503",
        "يخصّ OWN-22 (توحيد الفوتر) جزئياً.",
    ),
]
...
def add_missing(item_model):
    ...
    created = []
    for offset, (code, lane, title, pr, *related) in enumerate(NEW_ITEMS):
        ...
        item_model.objects.create(
            ...
            pr=pr,
            note=" ".join([f"{STAMP} عملٌ دُمج بلا بندٍ في الخارطة، سُجّل مُغلقاً برقم طلبه.", *related]),
