...
- **REP-01 → منجز بعمليّةٍ لا بطلب دمج** (بإذن المالك، كسابقة DBT-05) بما أفادت به جلسةُ Git، ثمّ وصل تفصيلُ معياره (الاستعادةُ التجريبيّة
  وعددُ النسخ).
- **REP-03 → قيد التنفيذ 60%** بحسابٍ صريحٍ من جلسة Git (3 من 5 خطواتٍ متساوية الوزن)، والمقارنةُ على الإنتاج بيد المالك وبوّابتُه باقية.
...
UPDATES = [
    (
        ...
        "(docs/governance/regression_guards.md). ونشأ منه دَينٌ: تحذيراتُ توثيق API المُسكَتة بمعرِّفَين (DBT-56). **منشورٌ على الإنتاج "
        "(main@db33f87، نحو 08:50 الدوحة، مع #589 و#567 بلا هجرات)؛ ولا أثرَ ظاهراً منه على الإنتاج.",
    ),
    (
        "REP-01",
        ("todo", 0),
        ...
        "حجمُها 125.5MB على الجهاز. **وصلني تفصيلُ معيار البند بعد ذلك:** الاستعادةُ التجريبيّةُ في مجلّدٍ معزولٍ طابقت مراجعَ الحزمة كلَّها "
        "(وتُجرى أيّامَ الجمعة ضمن المهمّة الليليّة)، ويُحتفظ بآخر 14 نسخة، والمهمّةُ الليليّةُ تعمل 02:00 بتوقيت الدوحة. **RK2 لا يتغيّر "
        "قبل REP-05:** النسخةُ على الجهاز نفسِه فليست نسخةً بعيدة.",
    ),
    (
        "REP-03",
        ("todo", 0),
        ...
        60,
        "",
        "**قيد التنفيذ 60%** بحسابٍ صريحٍ من جلسة Git (2026-09-25): خمسُ خطواتٍ متساوية الوزن، منجزٌ منها ثلاث — (أ) التحقّقُ من طريقة "
        "المقارنة محلّيّاً ✔، (ب) أداةُ المقارنة مبنيّةٌ ومُختبَرةٌ بقيمٍ اصطناعيّة (8 فحوصٍ ناجحة) ✔، (ج) آليّةُ التنفيذ عن بُعد مجرَّبةٌ "
        "بأمرٍ عديم الضرر على خدمةٍ واحدة ✔؛ والباقي: (د) تشغيلُ المقارنة على الخدمات الثلاث (0 من 3) وينفّذه المالكُ بنفسه، و(هـ) تسجيلُ "
        "الأحكام. تتغيّر النسبةُ بعد (د). البوّابةُ «المالك» باقية.",
    ),
]
