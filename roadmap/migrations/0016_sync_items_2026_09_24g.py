...
- **DBT-39 → منجز** بـ#559 (تلاشي الصفحات).
...
UPDATES = [
    ...
    (
        ...
        100,
        "#559",
        "أُغلق: #559 أُصلح المحدِّدُ الناقص (`> {`) في قواعد الانتقال الثلاث، وحارسٌ يمنع المحدِّداتِ الناقصة؛ فعاد تلاشي المحتوى 500ms "
        "عند التنقّل بين الصفحات (لم يعمل منذ #423). منشورٌ على الإنتاج ضمن main@25d0f62. **قِيس (Chromium، خادمُ جلسةٍ محلّيّ):** المحدِّدُ "
        "المعطوب 0 قاعدة والمُصلَح 1؛ التلاشي 500ms ظهوراً واختفاءً نهاراً وليلاً؛ وprefers-reduced-motion تُبطل الحركة؛ ومسحُ 74 صفحةً "
        "فريدةَ البنية (220 ابناً مباشراً): الظهورُ على الكلّ، والخروجُ ناقصٌ عند 4 أبناءٍ (.sa-panel في ثلاث صفحات و.schedule-pages-card) "
        "عولجوا بفرض انتقال الخروج بـ!important؛ وأثرُ الطلب على CSS الخامّ +328 بايتاً. الحارسُ tests/test_css_selectors.py. "
        "**مُتحقَّقٌ من الأصل على الإنتاج** (جلسةُ النشر، بـcurl): 20-components.705047548cfb.css المبصومُ يحوي القاعدةَ المصلَحة نصّاً ولا يوجد المحدِّدُ المبتور في أيٍّ من CSS الثمانية. **لم يُقَس:** حلقةُ page-nav.js الكاملة بجلسةٍ حقيقيّة، ولا رسمُ التلاشي في المتصفّح على الإنتاج — فلا يُوسَم «تحقّق رسمٍ» إلا بقياس المالك. "
        "أثرٌ محسوس: الأبناءُ المباشرون المخفيّون (.rm-panel في تبويبات الخارطة، modal-overlay، htmx-indicator) يتلاشون 500ms عند "
...
NEW_ITEMS = [
    ...
    (
        ...
        "الجدول الأسبوعيّ الديناميكيّ (يبقى مفتوحاً، تصميمُه ينتظر المالك). قياسُ التعبئة على الإنتاج: 2829 من 3271 حصّة معبَّأة (86%) و442 فارغة "
        "(تاريخيّة). (أبلغت به جلسةُ «اصلاحات 01»؛ منشورٌ ضمن main@25d0f62.)",
    ),
]
...
OPEN_DEBTS = [
    ...
    (
        ...
        "أبلغته جلسةُ إصلاح الأيقونة (14 شبكةً فارغة)؛ وعدّدتُ **10** شبكاتِ schoolos-*_default حينها. ثمّ فُرّغت الشبكاتُ من طرفٍ آخر قبل "
        "أن يُحذف شيءٌ بإذن (33 ← 19 شبكة، ولا شبكةً فارغةً الآن بحسب الجلسة) — فالعَرَضُ زال والسببُ (لا تنظيفَ آليَّ للجلسات "
        "المتوقّفة) باقٍ.",
    ),
    (
        "DBT-53",
