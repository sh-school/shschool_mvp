...
- **VI-34 ← #555** (أيقونةُ حوار التأكيد المكسورة)، و**N-036 ← #563**، و**N-037 ← #547**، و**N-038 ← #562** (رقمُ الحصّة محفوظٌ في Session).
...
UPDATES = [
    ...
    (
        ...
        100,
        "",
        "بعده وحّد #547 ألوانَ القائمة والذيل والقوائم المنسدلة برموزٍ مركزيّة (--nav-* و--menu-* و--footer-bg) نهاراً وليلاً في المنصّة "
        "وadmin_theme.css معاً — انظر N-037.",
    ),
    (
        "OWN-22",
        ("done", 100),
        ...
        100,
        "",
        "بعده جعل #547 الذيلَ بلون الترويسة (--footer-bg = --header-bg) وما يقرأ عليه — انظر N-037.",
    ),
    (
        "SCH-11",
        ("todo", 0),
    ...
    (
        ...
        0,
        "",
        "بعد #555: CSS الخامّ 464,743 بايتاً من سقف 470KB (هامشٌ نحو 5.3KB)؛ وبعد #562: 468.71KB (هامشٌ ضيّق 1.3KB) — فأيُّ CSS قادمٍ يكاد يبلغ السقف (والخطّةُ تقترح VD8: لا رفعَ ثانياً).",
    ),
    (
        "Q-11",
        ("todo", 0),
...
NEW_ITEMS = [
    (
        "VI-34",
        "frontend",
        "أيقونةُ حوار «تأكيد الإجراء»: base.js يشير إلى #icon-alert-triangle من ورقةٍ محذوفة (37 موضعاً في 25 قالباً) + اختبارٌ يمسح مراجع الأيقونات في JS",
        "#555",
    ...
    (
        ...
        "الجدول الأسبوعيّ الديناميكيّ (يبقى مفتوحاً، تصميمُه ينتظر المالك). قياسُ التعبئة على الإنتاج (قراءةٌ فقط، جلسةُ النشر 22:40 الدوحة): 2829 من 3271 حصّة معبَّأة (86%) و442 فارغة، **كلُّها تاريخيّة**: صفرٌ منها بتاريخ اليوم فما بعده فالأسابيعُ القادمة تحمل رقمها كلُّها، وتواريخُها من 2026-08-31 إلى 2026-09-03 (الأسبوعُ الأوّل من الدوام، جدولٌ أُبدل قبله)، ولا شُعبةَ بلا time_band. (أبلغت به جلسةُ «اصلاحات 01»؛ منشورٌ ضمن main@25d0f62.)",
    ),
    (
        "N-037",
        "frontend",
        "الهويّة: القائمةُ رملٌ خفيف والذيلُ بلون الترويسة والقوائمُ المنسدلة بلون القائمة — رموزٌ مركزيّةٌ فقط",
        "#547",
        "لا قالبَ ولا صفحةً عُدّلت: --nav-bg/--nav-fg/--nav-hover/--nav-mark، و--footer-bg = --header-bg وما يقرأ عليه، و--menu-bg = "
        "--nav-bg وأخواتها، و--shadow-menu، وأسماءُ أعمدة القوائم --menu-label، نهاراً وليلاً في المنصّة وadmin_theme.css. الحرّاس: "
        "test_admin_theme (مرآةُ الرموز وقوائمُ وظلٌّ وذيل، وحقلُ بحث الإدارة يقرأ رموزَ القائمة) وأزواجُ تباينٍ جديدة في "
        "test_contrast_ratios. ودليلُ الهويّة: مفتاحُ صفحاته الثلاث الدائم ومجموعةُ رموزٍ وقسمُ 23 بمعاينةٍ حيّة. **مقيس:** axe في "
        "Chromium على صفحات الإدارة الخمس نهاراً وليلاً = صفر، وحمولةُ CSS الخامّ لم تتغيّر أثراً يُذكر. لم يُقَس الرسمُ على الإنتاج. "
        "(أبلغت به جلسةُ «ألوان القائمة الرئيسية والفوتر» 8104، وهو يوسّع ما وحّده OWN-09 وOWN-22 قبله؛ وقيدُ Dune على «رمل القائمة» "
        "ما زال بلا جواب.)",
    ),
    (
        "N-038",
        "desktop",
        "تلاشي القوائم المنسدلة من لحظة النقر، ونقلُ آليّة الانتقال بين الصفحات إلى لوحة الإدارة",
        "#562",
        "القائمةُ المنسدلةُ تتلاشى عند النقر بمدّة --transition-page نفسِها ثمّ تُغلق (page-nav.js::fadeMenus، و.sd-menu.is-fading في "
        "المنصّة، و.adm-nav__item.is-fading في الإدارة) — كانت تبقى مفتوحةً حتى تبديل المحتوى ثمّ تختفي فجأة. وفي /admin/: page-nav.js بنمط "
        'data-page-nav="fade" بلا تبديل DOM، مع @view-transition في admin_theme.css (Chrome/Edge 126+ وSafari 18.2+ يمازجان الصفحتين فلا '
        "ترمش الترويسةُ والذيل) واحتياطِ تلاشٍ بـJS لغيرها، وخطوطُ Tajawal مسبقةُ التحميل؛ والرمزُ --transition-page وحركةُ page-in منقولان "
        "حرفيّاً ويحرسهما test_admin_theme. تحصينٌ: is-leaving لا يبقى على صفحةٍ مُستعادةٍ من bfcache ولا بعد تنزيلٍ بلا تنقّل. **مقيس:** "
        "فحوصُ CI 23/23، والحرّاسُ المحلّيّة (admin_theme 23، page_nav_shell 12، css_selectors 21، وغيرها)، وCSS الخامّ 468.71KB من 470KB. "
        "**لم يُقَس (لا يُوسَم منجزاً بصريّاً):** المزجُ البصريّ الفعليّ في الإدارة، وأثرُ opacity: 0 عند التحميل على LCP في Chrome "
        "حقيقيّ — ويحتاج معاينةَ المالك لـ/admin/ في Chrome/Edge. وإصلاحُ المحدِّدات المقطوعة وحارسُه هما #559 (DBT-39) لا هذا. "
        "(أبلغت به جلسةُ إصلاح المحدِّدات المقطوعة؛ وهو يسبق VI-36 ويستفيد منه.)",
    ),
]
NEW_ITEMS_FIRST_ORDER = 470  # بعد N-034 (469)
# VI-34 بندٌ من خطّة إصلاح الهويّة (عائلةُ VI) أُغلق قبل أن تُدرج الخطّةُ كلُّها؛ وغيرُه عملٌ دُمج بلا بند.
ITEM_SRC = {"VI-34": "VI"}
ITEM_NOTE = {"VI-34": "بندٌ من خطّة إصلاح الهويّة البصريّة (تُدرج في الهجرة التالية) أُغلق برقم طلبه."}
...
def add_missing(item_model):
    ...
    for offset, (code, lane, title, pr, extra) in enumerate(NEW_ITEMS):
        if item_model.objects.filter(code=code).exists():
            continue
        note = f"{STAMP} " + ITEM_NOTE.get(code, "عملٌ دُمج بلا بندٍ في الخارطة، سُجّل مُغلقاً برقم طلبه.")
        item_model.objects.create(
            code=code,
            src=ITEM_SRC.get(code, "NEW"),
            lane=lane,
            title=title,
            status="done",
            progress=100,
