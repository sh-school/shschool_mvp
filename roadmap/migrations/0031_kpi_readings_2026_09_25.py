...
- **تصحيحُ N-043:** المالكُ منح القدرةَ لمعلّمَين بيده مساء 2026-09-25 (منحان فعّالان بحسب أداة المنح نفسِها)، فتسقط عبارةُ «لم يُمنح أحدٌ شيئاً بعد» من 0030.
...
ITEM_NOTES = [
    ...
    (
        "N-043",
        "قياسُ الجزء (أ) (أمين البلاغات، 2026-09-25): منح المالكُ بيده مساء اليوم على الإنتاج قدرةَ schedule.operator لمعلّمَين بأمر grant_capability "
        "(مدقَّقٌ باسمه مع السبب)، وأداةُ العرض (--list) تعرض **منحَين فعّالين** بعد أن كانا 0 عند نشر #620. الأثرُ الفعليّ الآن: إسنادُ المواد لكلّ الأقسام "
        "لهما بلا تأثّرٍ بوقف المنسّقين (#509). أمّا توليدُ الجدول وربطُه بالقدرة والنافذةُ وسلسلةُ الاعتماد فبعد إقلاع الأحد (جلسةُ الجدولة) — فلا يُوسم "
        "البندُ منجَزاً كلّياً.",
    ),
]


# تصحيحُ عبارةٍ صارت خاطئةً في حقلٍ نصّيٍّ من بندٍ: (الرمز، الحقل، النصُّ القديم حرفيّاً، النصُّ الجديد)
FIELD_CORRECTIONS = [
    (
        "N-043",
        "criterion",
        "(لم يُمنح أحدٌ شيئاً بعد)",
        "(مُنحت لمعلّمَين بيد المالك مساء 2026-09-25: منحان فعّالان بحسب أداة المنح نفسِها)",
    ),
    (
        "N-043",
        "note",
        "يبقى منحُ القدرة (خطوةُ المالك بالأمر بعد الدوام) والجزءان ب وج",
        "منحُ القدرة لمعلّمَين تمّ بيد المالك مساء 2026-09-25 (بعد أن كان عددُ المنح 0 عند النشر)؛ ويبقى الجزءان ب وج",
    ),
]
...
def correct_fields(item_model):
    """تستبدل عبارةً صارت خاطئةً حرفيّاً في معيارٍ أو ملاحظة؛ تُرجع رموزَ ما تغيّر."""
    changed = []
    for code, field, old, new in FIELD_CORRECTIONS:
        item = item_model.objects.filter(code=code).first()
        if item is None or old not in getattr(item, field):
            continue
        setattr(item, field, getattr(item, field).replace(old, new))
