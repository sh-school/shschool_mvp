...
حارسةٌ كـ0003..0006: كلُّ إنشاءٍ يتخطّى ما وُجد بالرمز، والتحديثُ لا يمسّ بنداً عدّله المطوّر
(الشرطُ حالتُه وتواريخُه ما في آخر لقطة)، والملاحظةُ لا تُلحَق ثانيةً.
...
# (الرمز، سطرُ الملاحظة، التواريخُ المتوقَّعة للتعديل، البدايةُ الجديدة، النهايةُ الجديدة)
NOTES = [
    (
        "U-33",
    ...
    (
        ...
        " بدايتُه ونهايتُه مقترَحتان: من 2026-12-01 إلى 2027-02-26.",
        (D(2026, 11, 2), D(2026, 12, 17)),
        D(2026, 12, 1),
        D(2027, 2, 26),
    ),
    (
...
def annotate(item_model):
    """تُلحِق ملاحظاتِ الربط على البنود القائمة؛ تُعدِّل التواريخ إن لم يمسّها المطوّر."""
    changed = []
    for code, line, expected_dates, start, end in NOTES:
        ...
        if start is not None:
            untouched = (
                item.status == "todo"
                and item.progress == 0
                and (item.start_date, item.end_date) == expected_dates
            )
            if not untouched:
                continue
            item.start_date, item.end_date = start, end
            item.date_basis = BASIS
