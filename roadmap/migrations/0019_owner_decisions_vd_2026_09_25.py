...
- **VD4 محسوم بالنطاق المعدَّل:** أعاد المالكُ نطاقَه في محادثة 8104 بعد جوابه على «الباقي» («موافق») ثمّ أكّده لجلسة الخارطة
  مباشرةً: ملفّاتُ الشعار الأصليّة من مدير المدرسة أو من المنصّة نفسها، والنسخةُ المعكوسة ومساحةُ الحماية نصنعهما بلا فلتر تبييض —
  فلا تنطبق توصيتُه السابقة «SVG ومعكوسٌ رسميّان». وD-14 وD-15 مفتوحان.
...
وبعد دفع 0018 اندمج طلبان ونُشرا على الإنتاج (main@02b68ed، بعد 06:12 الدوحة 2026-09-25) ويُسجَّلان هنا:

- **#571** (8096) ملاحظةٌ على OWN-19: هجرة core/0072 (بياناتٌ فقط، لا رجوع) — **منشورٌ ومطبَّق** بتأكيد المالك المباشر لجلسة النشر بعد
  اطّلاعه على الرقم 16: حُذفت 8 صلاحيّاتٍ (600 ← 592) وسقط معها 16 إسناداً مباشراً لحسابَي مستخدمَين، وصفرٌ للمجموعات.
- **#577** (8061) الطلبُ الأوّلُ من ثلاثةٍ للجدول الأسبوعيّ الديناميكيّ: **N-041 قيد التنفيذ 1/3** (عدُّ طلباتٍ لا جهد)، منشورٌ
  بلا أثرٍ مرئيّ (قراءةٌ محضةٌ غيرُ موصولةٍ بصفحة). وD-21 يبقى مفتوحاً للنقاش بطلب المالك.
...
DECISIONS = [
    ...
    (
        ...
        "open",
        "decided",
        "حسمه المالكُ بالنطاق المعدَّل (2026-09-25): أعاد نطاقَه في محادثة جلسة 8104 بعد جوابه على «الباقي» («موافق») ثمّ أكّده لجلسة "
        "الخارطة مباشرةً. **يحلّ محلّ التوصية السابقة «SVG ومعكوسٌ رسميّان»:** ملفّاتُ الشعار الأصليّة من مدير المدرسة أو من المنصّة "
        "نفسها، والنسخةُ المعكوسة ومساحةُ الحماية نصنعهما بلا فلتر تبييض؛ ويُقرأ V-K39 «نسخُ الشعار الأصليّة». **الحسمُ قرارٌ لا "
        "تنفيذ** — VI-11 يبقى مفتوحاً وموعدُه لم يُغيَّر.",
    ),
    (
        "VD5",
        "open",
...
UPDATES = [
    (
        ...
        "حارسُه tests/test_delete_orphan_permissions_migration.py (7 حالات). **منشورٌ ومطبَّق على الإنتاج (main@02b68ed، بعد 06:12 "
        "الدوحة 2026-09-25) بتأكيد المالك المباشر لجلسة النشر بعد اطّلاعه على الرقم 16، وقياسُ ما بعد النشر مطابقٌ لقياس ما قبله:** "
        "حُذفت 8 صلاحيّاتٍ يتيمةٍ (4 لكلٍّ من النموذجَين)، والإجماليّ 600 ← 592، والإسناداتُ المباشرةُ المحذوفةُ 16 لحسابَي مستخدمَين "
        "(8 لكلٍّ) و0 للمجموعات؛ والأزواجُ مسجَّلةٌ في سجلّ النشر فتُستعاد يدويّاً إن لزم. فحصُ ما بعد النشر: /health/ سليم، الدخول 200، "
        "صفرُ أخطاءٍ في الخدمات الثلاث، ولا هجراتٍ غيرَ مطبَّقة.",
    ),
]
...
NOTES = [
    ...
    (
        "VI-11",
        ("todo", 0),
        "حُسم VD4 (2026-09-25) بالنطاق المعدَّل: ملفّاتُ الشعار الأصليّة من مدير المدرسة أو من المنصّة نفسها، والنسخةُ المعكوسة "
        "ومساحةُ الحماية نصنعهما بلا فلتر تبييض — قرارٌ لا تنفيذ، ولا يصحّ ما كُتب هنا من «SVG ومعكوسٌ رسميّان». **ما تحقّقتُ منه "
        "في المستودع:** static/brand/logoMaroon.png (295×295) ونسختُه البيضاء logowhite.png (295×295)، وstatic/icons/favicon.png "
        "(256×256)، ولا SVG لشعار المدرسة في static/brand (الموجود azkia-logo.svg للشركة لا للمدرسة). **وبنقل 8104 ولم أقِسه:** "
        "الأصلُ لونٌ واحدٌ مسطّح #8A1538 والنسخةُ البيضاء بالشكل نفسه، والحاليُّ في الترويسة والدخول والإدارة والأخطاء وfavicon وPWA "
        "هو icons/favicon.png (مظلَّلة، تطابقُ الشكل مع الأصل 0.40). فمعيارُ VI-11 المقترَح: المرجعُ logoMaroon.png وتُشتقّ منه "
        "أيقوناتُ الواجهة (وSVG من الأصل المسطّح)، وتُستعمل logowhite.png بدل فلاتر التبييض.",
    ),
    (
        "VI-38",
        ("todo", 0),
...
IN_PROGRESS_ITEMS = [
    (
        ...
        "في tests/test_week_schedule.py و60 خضراء مع صفحات الجدول القائمة؛ ruff 0.4.4 وRadon وmypy بإصدار CI. **دُمج ونُشر على "
        "الإنتاج (main@02b68ed) ولا واجهةَ فيه: قراءةٌ محضةٌ غيرُ موصولةٍ بأيّ صفحة فلا أثرَ ظاهراً ولا يقرأ صفحةَ الجدول من Session؛ "
        "وتحقّقُه بالاختبارات لا بفحصٍ إنتاجيٍّ حيّ.** القرارُ D-21 مفتوحٌ للنقاش بطلب المالك: "
