...
- **REP-18 وU-19 → قيد التنفيذ** (#618): 75% و33% اقتراحا صاحبهما 8204 (اشتقاقٌ لا قياس)؛ **REP-18 لا يُغلق قبل تحقّق خضرة Nightly وQuality الأسبوعيّ.**
- **ملاحظاتٌ:** D-23 (#610) نُفِّذ وأُلحق بـLAY-08 وقرارِه.
"""
...
UPDATES = [
    ...
    (
        ...
        False,
        "#618",
        "**#618 دُمج (931dc4aa) ونُشر ضمن main@a213d3c نحو 18:58 الدوحة — لا يُغلق قبل التحقّق:** المُنفَّذ (8204/8097): quality.yml الأسبوعيّ يثبّت متصفّحات "
        "Playwright ويعزل اختباراتها في خطوةٍ منفصلة، ومهلةٌ لـmutmut وvulture (كان mutmut يدور ستَّ ساعاتٍ فيُلغى)، وحارسُ العزل يقرأ الملفّين، وحارسٌ "
        "report-only، واختبارُ التنقّل بلا تاريخٍ مثبَّت مع نظيرٍ سريعٍ في بوّابة الطلبات، وإصلاحُ علّةٍ حقيقيّةٍ في القالب كشفها الاختبار (معرّفٌ مكرَّرٌ "
        "`#teacher-schedule-row` بلا hx-swap في observation_form). **المقيس حتى الآن (8204، 2026-09-25):** تشغيلُ quality.yml اليدويّ على الفرع أخضرُ كلُّه — "
        "المجموعةُ 8111 ناجحاً وتغطيةٌ 86.99% (العتبة 70)، وmutmut بالمهلة أخضر، وخطوةُ Playwright المنفصلة خضراء؛ والطلبُ 23/23 فحصاً. **لم يُقَس بعد** "
        "(وهو معيارُ البند): خضرةُ Nightly المجدول التالي (02:00–08:00 UTC من 09-26) وQuality الأسبوعيّ (الأحد 09-27)، وإغلاقُ #422 بعد الخضرة؛ ومصدرُ الحقيقة "
        "`gh run list --workflow nightly.yml`. **التقدّم 75% = 3 من 4 معايير — اقتراحُ صاحبه (8204) لا قياس.**",
    ),
    (
        "U-19",
        ("todo", 0),
        ...
        False,
        "#618",
        "**U-19: جزءٌ من ثلاثة منجز — #618 أصلح سطرَ CLAUDE.md** (الإشارةُ إلى railway-migrate.sh غيرِ الموجود ← railway-predeploy.sh، بإذن المالك؛ على main "
        "ومنشورٌ ضمن main@a213d3c؛ تحقّقتُ أنّه في تاريخ main). **يبقى:** حذفُ Grafana وتثبيتُ sentry-sdk وتصحيحُ docs/protocols (وسكربتا backup/restore "
        "القديمان). **التقدّم 33% = 1 من 3 — اقتراحُ صاحبه (8204) لا قياس؛** ولا يُغلق البند.",
    ),
]
