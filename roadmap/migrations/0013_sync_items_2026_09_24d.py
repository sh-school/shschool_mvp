...
- **D-18** قرارٌ جديدٌ محسومٌ بتاريخه: «لا، الفسحة والصلاة تفصلان، لا تُعدّ تلاصقاً» (أكّده المالك في محادثة
  الخارطة: «نعم لا تعد تلاصق اذا فصلتهما فسحة او صلاة»). فالتلاصقُ يُحكم بالساعة، وبندُه التنفيذيّ SCH-18.
...
ADJACENCY_RULING = "لا، الفسحة والصلاة تفصلان، لا تُعدّ تلاصقاً"
...
NEW_ITEMS = [
    ...
    (
        ...
        "SCH",
        "backend",
        "التلاصقُ بالساعة: الفسحةُ والصلاة تفصلان الحصّتين",
        "todo",
        0,
        "",
        "من حسم D-18 (2026-09-24). يشمل HC5 وSC1 والمحسّنَ ومؤشّرَ run_breaches والمدقّق؛ والحصّتان متّصلتان إن كان "
        "الفاصلُ عشرَ دقائق فأقلّ (كتعريف المزدوجة). وبه يتغيّر ما يعدّه SK3 (أيّامُ التلاصق المخالف)، فيُعاد قياسُه "
        "بعد اندماجه. رقمُ طلبه حين يُفتح.",
        D(2026, 9, 24),
        D(2026, 9, 26),
        "مقترَح — من خطّة جلسة الجدولة، لم يعتمده المالك",
        618,  # بعد SCH-17 (617)
...
DETAILS = {
    ...
    "SCH-18": (
        0.5,
        "D-18",
        "13 من 46 «تلاصقاً» في الجدول المعتمد تعبر فسحةً أو صلاة فتسقط من العدّ، باختبار؛ والفاصلُ المتّصل ≤ 10 دقائق.",
        "docs/schedule_generation_remediation_plan_2026-09.md",
    ),
}
...
NEW_DECISIONS = [
    {
        "code": "D-18",
        "src": "SCH",
        "title": "التلاصق: هل تفصل الفسحةُ والصلاة بين حصّتين، أم تُعدّان متلاصقتين؟",
        "status": "decided",
        "decider": "المالك",
        "due": "2026-09-24",
        "decision_date": DAY,
        "blocks": "SCH-18",
        "options": "(أ) تفصلان: التلاصقُ يُحكم بالساعة، والحصّتان متّصلتان إن كان الفاصلُ عشرَ دقائق فأقلّ. "
        "(ب) لا تفصلان: الحصّتان المتتاليتان في الترتيب متلاصقتان أيّاً كان بينهما.",
        "recommendation": f"{STAMP} حسمُ المالك: «{ADJACENCY_RULING}» — (أ). أثرُه: 13 من 46 «تلاصقاً» في الجدول "
        "المعتمد تعبر فسحةً أو صلاة فتسقط من العدّ.",
        "sort_order": 116,  # بعد D-17 (115)
    },
]
