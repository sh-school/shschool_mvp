"""عزلُ جدولَي حضور الموظّفين والأذونات على مستوى الصفّ في PostgreSQL.

`core.0037` فعّل العزلَ على كلّ جدولٍ يحمل `school_id` يومَ كُتب، فجدولٌ يُنشأ
بعده يخرج بلا سياسة (انظر صدرَ `core/migrations/0063_rls_wing.py`). والجدولان
هنا يحملان `school_id` بنفسيهما، فالسياسةُ مباشرة، والهويّةُ من
`app_rls_school()` — دورُ الاتصال لا متغيّرُ جلسة.
"""

from django.db import migrations

CURRENT = "public.app_rls_school()"
TABLES = ("staff_affairs_staffattendance", "staff_affairs_permitrequest")


def _enable(table: str) -> str:
    predicate = f"{table}.school_id = {CURRENT}"
    return f"""
ALTER TABLE public.{table} ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS school_isolation ON public.{table};

CREATE POLICY school_isolation ON public.{table}
    USING ({predicate})
    WITH CHECK ({predicate});
"""


def _disable(table: str) -> str:
    return f"""
DROP POLICY IF EXISTS school_isolation ON public.{table};
ALTER TABLE public.{table} DISABLE ROW LEVEL SECURITY;
"""


class Migration(migrations.Migration):
    dependencies = [
        ("staff_affairs", "0003_staff_attendance_and_permits"),
        ("core", "0037_rls_tenant_identity_from_db_role"),
    ]

    operations = [
        migrations.RunSQL(sql=_enable(table), reverse_sql=_disable(table)) for table in TABLES
    ]
