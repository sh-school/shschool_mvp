    ...
    * النائبُ الإداريّ يكلّف أحدَ من تحت مسؤوليته (مسؤولُهم المباشرُ في بطاقاتهم) بأعبائه.
    ...
    def revoke(
        ...
        if assignment.revoked_at is not None:
            return assignment
        assignment.revoked_at, assignment.revoked_by = _now(), actor
...
class StaffAttendanceService:
    ...
    def manages(
        ...
        if manager not in (PRINCIPAL_DELEGATE, "vice_academic") or user.pk == staff.pk:
            return False
        return user.pk in (day or _StageDay(school, _now())).holders(manager)
