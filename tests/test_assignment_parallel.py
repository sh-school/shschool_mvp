...
# ════════════════════ تباعدُ الأيّام — بنطاق مرحلة الشعبة ════════════════════


def spread(client, row, on):
    return client.post(
        reverse("academic_management:assignment_toggle_spread", args=[row.id]),
        {"spread": "1", "year": YEAR} if on else {"year": YEAR},
        HTTP_HOST="localhost",
    )


def test_spreading_a_secondary_row_scopes_to_secondary(client, school, group, teacher, principal):
    art = assign(school, group, teacher, "الفنون البصرية", 2)
    client.force_login(principal)

    spread(client, art, True)

    art.subject.refresh_from_db()
    assert art.subject.spread_days_scope == "sec"


def test_the_other_level_is_left_alone(client, school, group, teacher, principal):
    """الحاسمُ: إشعالُه في الثانويّ لا يرفع التباعدَ عن الإعداديّ ولا يفرضه."""
    art = assign(school, group, teacher, "الفنون البصرية", 2)
    art.subject.spread_days_scope = "prep"
    art.subject.save(update_fields=["spread_days_scope"])
    client.force_login(principal)

    spread(client, art, True)
    art.subject.refresh_from_db()
    assert art.subject.spread_days_scope == "all"

    spread(client, art, False)
    art.subject.refresh_from_db()
    assert art.subject.spread_days_scope == "prep"


def test_an_impossible_spread_is_refused_at_save(client, school, group, teacher, principal):
    """مادّةُ ستِّ حصصٍ لا تتباعد في خمسة أيّام — تُردّ عند الحفظ لا بعد التوليد."""
    maths = assign(school, group, teacher, "الرياضيات", 6)
    client.force_login(principal)

    spread(client, maths, True)

    maths.subject.refresh_from_db()
    assert maths.subject.spread_days_scope == "none"
