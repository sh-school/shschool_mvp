...
class TestReportInfractionExtended:
    ...
    def test_post_non_numeric_level(
        self, client_as, teacher_user, school, student_user, enrolled_student
    ):
...
class TestCommitteeDashboardExtended:
    ...
    def test_committee_stats_counts(
        self, client_as, principal_user, school, student_user, teacher_user
    ):
        BehaviorInfractionFactory(
            school=school,
    ...
    def test_committee_resolved_cases(
        self, client_as, principal_user, school, student_user, teacher_user
    ):
        BehaviorInfractionFactory(
            school=school, student=student_user, reported_by=teacher_user, level=3, is_resolved=True
        )
        client = client_as(principal_user)
        resp = client.get("/behavior/committee/")
...
class TestCommitteeDecisionExtended:
    ...
    def test_escalate_level4_stays_4(self, client_as, principal_user, level4_infraction):
        client = client_as(principal_user)
        client.post(
            f"/behavior/committee/{level4_infraction.id}/decision/",
            {
                "decision": "escalate",
            },
        )
...
class TestPDFViews:
    ...
    def test_warning_pdf_renders(
        self, mock_pdf, client_as, principal_user, school, behavior_infraction
    ):
    ...
    def test_policy_pdf_exists(self, mock_open, mock_exists, client_as, teacher_user, school):
