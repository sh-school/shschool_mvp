"""صفحةٌ لكلّ معلّمٍ أو لكلّ شعبة — وقسمُ المعلّم من سجلّ المدرسة.

قرارُ الإدارة 2026-09-06: القسمُ ما يقوله السجلّ **بغضّ النظر عمّا يدرّسه
المعلّم**. فالورقةُ تُوزَّع على المنسّقين، ورجلٌ ينتقل من قسمٍ إلى قسمٍ لأنّ
جدوله تغيّر ورقةٌ لا تُصدَّق. وكان الاشتقاقُ من الموادّ هو المصدرَ الوحيد.

ومعها: منسّقُ المشاريع الإلكترونيّة يعاين الجداول كالمنسّق، والتخصّصُ المكتوب
يُذكر بجانب اسم صاحبه، ولا صفحةَ لمن لا حصّةَ له، ولا فهرسَ في أوّل الورق.
"""

from datetime import time

import pytest
from django.urls import reverse

from core.models import Department, Membership
from operations.models import ScheduleSlot, Subject
from operations.services import ScheduleService
from tests.conftest import ClassGroupFactory

pytestmark = pytest.mark.django_db
YEAR = "2026-2027"


@pytest.fixture
def arts(school):
    return Department.objects.create(
        school=school, name="الفنون البصرية", code="art", sort_order=10
    )


def _link(user, school, department, specialty=""):
    membership = Membership.objects.filter(user=user, school=school, is_active=True).first()
    membership.department_obj = department
    membership.specialty = specialty
    membership.save(update_fields=["department_obj", "specialty"])
    return membership


def _lesson(school, teacher, subject_name="الرياضيات", code="MAT", *, day=0, period=1, group=None):
    subject, _ = Subject.objects.get_or_create(school=school, name_ar=subject_name, code=code)
    return ScheduleSlot.objects.create(
        school=school,
        class_group=group
        or ClassGroupFactory(school=school, grade="G8", level_type="prep", academic_year=YEAR),
        teacher=teacher,
        subject=subject,
        day_of_week=day,
        period_number=period,
        start_time=time(7, 30),
        end_time=time(8, 15),
        academic_year=YEAR,
        is_active=True,
    )


# ══════════════════════ القسم من السجلّ ═══════════════════════════


def test_the_registered_department_beats_what_he_teaches(school, teacher_user, arts):
    """يدرّس الرياضيات ومسجَّلٌ في الفنون — فالورقةُ تقول الفنون."""
    _lesson(school, teacher_user)
    _link(teacher_user, school, arts)

    pages = ScheduleService.teacher_pages(school, YEAR)

    assert [p["department"]["name"] for p in pages] == ["الفنون البصرية"]
    assert pages[0]["department"]["registered"] is True


def test_an_unregistered_teacher_still_gets_a_department(school, teacher_user):
    """الاشتقاقُ احتياطٌ لا أصل — ومن لا سجلَّ له لا يسقط من الورق."""
    _lesson(school, teacher_user)

    pages = ScheduleService.teacher_pages(school, YEAR)

    assert pages[0]["department"]["name"] == "الرياضيات"
    assert pages[0]["department"]["registered"] is False


def test_the_specialty_and_the_head_ride_with_the_page(school, teacher_user, principal_user, arts):
    """مدرّسُ إدارة الأعمال وحدَه: قسمُه بقرار المدير، وتخصّصُه بجانب اسمه."""
    arts.head = principal_user
    arts.save(update_fields=["head"])
    _lesson(school, teacher_user)
    _link(teacher_user, school, arts, specialty="إدارة الأعمال")

    page = ScheduleService.teacher_pages(school, YEAR)[0]

    assert page["department"]["specialty"] == "إدارة الأعمال"
    assert page["department"]["head"] == principal_user.full_name


# ══════════════════════ النطاق ════════════════════════════════════


def test_a_department_scope_keeps_only_its_members(school, teacher_user, coordinator_user, arts):
    _lesson(school, teacher_user)
    _lesson(school, coordinator_user, day=1)
    _link(teacher_user, school, arts)

    pages = ScheduleService.teacher_pages(school, YEAR, department="art")

    assert [p["teacher"] for p in pages] == [teacher_user]


def test_a_single_teacher_scope_is_one_page(school, teacher_user, coordinator_user):
    _lesson(school, teacher_user)
    _lesson(school, coordinator_user, day=1)

    pages = ScheduleService.teacher_pages(school, YEAR, teacher_id=teacher_user.id)

    assert [p["teacher"] for p in pages] == [teacher_user]


def test_a_teacher_without_lessons_has_no_page(school, teacher_user, arts):
    _link(teacher_user, school, arts)

    assert ScheduleService.teacher_pages(school, YEAR) == []


def test_the_department_list_comes_from_the_timetable(school, teacher_user, coordinator_user, arts):
    """قسمٌ مسجّلٌ بلا معلّمٍ مجدول لا يظهر، والمشتقُّ لمن لا سجلَّ له يظهر."""
    Department.objects.create(school=school, name="الفيزياء", code="physics", sort_order=12)
    _lesson(school, teacher_user)
    _lesson(school, coordinator_user, "اللغة العربية", "ARA", day=1)
    _link(teacher_user, school, arts)

    names = [d["name"] for d in ScheduleService.department_options(school, YEAR)]

    assert "الفيزياء" not in names
    assert names == ["الفنون البصرية", "اللغة العربية"]


# ══════════════════════ الشُّعب ═══════════════════════════════════


def test_class_pages_come_in_school_order_with_the_teacher_in_the_cell(
    school, teacher_user, coordinator_user
):
    senior = ClassGroupFactory(school=school, grade="G12", level_type="sec", academic_year=YEAR)
    junior = ClassGroupFactory(school=school, grade="G7", level_type="prep", academic_year=YEAR)
    _lesson(school, teacher_user, group=senior, day=2, period=3)
    _lesson(school, coordinator_user, group=junior)

    pages = ScheduleService.class_pages(school, YEAR)

    assert [p["class_group"] for p in pages] == [junior, senior]
    assert pages[1]["by_period"][2][2][0].teacher == teacher_user


# ══════════════════════ الشبكة تُقلب للورقة ═══════════════════════


def test_the_page_grid_is_period_by_day(school, teacher_user):
    _lesson(school, teacher_user, day=2, period=3)

    by_period = ScheduleService.teacher_pages(school, YEAR)[0]["by_period"]

    assert len(by_period) == 7 and len(by_period[0]) == 5
    assert by_period[2][2] and not by_period[0][0]


# ══════════════════════ الصفحات والصلاحيّة ═══════════════════════


@pytest.mark.parametrize(
    "fixture", ["principal_user", "coordinator_user", "e_projects_coordinator_user"]
)
def test_the_leadership_and_both_coordinators_may_open_it(client, school, request, fixture):
    client.force_login(request.getfixturevalue(fixture))

    assert client.get(reverse("schedule_pages")).status_code == 200
    assert client.get(reverse("schedule_pages_paper")).status_code == 200


def test_a_plain_teacher_may_not_browse_the_school(client, school, teacher_user):
    client.force_login(teacher_user)

    assert client.get(reverse("schedule_pages")).status_code in (302, 403)


def test_the_paper_shows_the_specialty_beside_the_name_and_no_index(
    client, school, principal_user, teacher_user, arts
):
    _lesson(school, teacher_user)
    _link(teacher_user, school, arts, specialty="إدارة الأعمال")
    client.force_login(principal_user)

    body = client.get(reverse("schedule_pages_paper"), {"year": YEAR}).content.decode()

    assert f'{teacher_user.full_name}</b> <span class="specialty">(إدارة الأعمال)' in body
    assert "فهرس" not in body


def test_the_orientation_is_carried_into_the_page_rule(client, school, principal_user):
    client.force_login(principal_user)

    landscape = client.get(
        reverse("schedule_pages_paper"), {"orient": "landscape"}
    ).content.decode()
    default = client.get(reverse("schedule_pages_paper"), {"orient": "sideways"}).content.decode()

    assert "size: A4 landscape" in landscape
    assert "size: A4 portrait" in default, "اتّجاهٌ مجهولٌ يعود إلى العموديّ"


def test_the_platform_page_frames_the_paper_with_the_same_selection(client, school, principal_user):
    client.force_login(principal_user)

    body = client.get(
        reverse("schedule_pages"), {"kind": "classes", "orient": "landscape"}
    ).content.decode()

    assert "kind=classes" in body and "orient=landscape" in body
    assert reverse("schedule_pages_paper") in body and reverse("schedule_pages_pdf") in body
