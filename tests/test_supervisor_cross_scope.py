...
class TestExamIncident:
    ...
    def test_the_back_to_session_link_hides_for_the_supervisor(
        ...
        body = client_as(supervisor).get(
            reverse("exam_control:incidents", args=[exam.pk])
        ).content.decode()
        assert f'href="{session_url}"' not in body

    def test_the_back_to_session_link_shows_for_the_principal(
        self, client_as, principal, exam
    ):
        session_url = reverse("exam_control:session_detail", args=[exam.pk])
        assert client_as(principal).get(session_url).status_code == 200

        body = client_as(principal).get(
            reverse("exam_control:incidents", args=[exam.pk])
        ).content.decode()
