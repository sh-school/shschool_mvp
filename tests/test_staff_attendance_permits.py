...
class TestPermits:
    ...
    def test_exactly_two_hours_passes_and_one_minute_more_is_refused(self, school, principal_user):
        """م-13 (السياسة 4.4): 120 دقيقة جائزة، و121 مرفوضة."""
        staff = _staff(school, 1)
        _permit(school, staff, FEB, time(10, 0), time(12, 0))
        assert PERMIT_MAX_MINUTES == 120
    ...
    def test_a_permit_that_exceeds_the_monthly_cap_is_refused(self, school, principal_user):
        """م-15 (السياسة 4.2): 420 دقيقةً في شهر تاريخ الإذن، والشهرُ التالي رصيدٌ جديد."""
        staff = _staff(school, 1)
    ...
    def test_pending_requests_count_against_the_cap_at_submission(self, school, principal_user):
        ...
        with pytest.raises(PolicyError, match="4.2"):
            _permit(school, staff, date(2026, 2, 5), time(7, 0), time(8, 1), "late_arrival", None)
    ...
    def test_the_secretary_records_the_balance_and_does_not_decide(self, school):
        ...
        assert (waiting.recorded_balance_minutes, waiting.secretary_by) == (60, secretary)
    ...
    def test_the_stage_list_follows_the_paper(self):
        """م-19: ``PERMIT_STAGES`` بترتيب مربّعات الورقة ([ن02] ص1)."""
        assert [key for key, _label in PERMIT_STAGES][:3] == [
            "secretary",
            "supervisor",
            "principal",
        ]
...
class TestSpecRules:
    ...
    def test_the_department_head_is_not_a_stage(self, school, principal_user):
        from core.models.department import Department

        coordinator = _staff(school, 20, role="coordinator")
        teacher, vice_academic = _staff(school, 1), _actor(school, "vice_academic")
        dept = Department.objects.create(
            school=school, name="قسم رياضيات", code="math", head=coordinator
        )
        Membership.objects.filter(school=school, user__in=[coordinator, teacher]).update(
            department_obj=dept
        )
        ...
        assert coordinator.pk not in _eligible(permit)
        assert _eligible(permit) == {vice_academic.pk}
        assert PermitService.awaiting(school, coordinator) == []
    ...
    def test_marking_the_principal_absent_hands_the_final_approval_to_the_admin_deputy(
        self, school, principal_user
    ):
        teacher = _staff(school, 1)
        ...
        assert vice_admin.pk not in _eligible(permit)  # المديرُ حاضرٌ فلا إنابة
    ...
    def test_the_academic_deputy_never_stands_in_for_the_principal(self, school, principal_user):
        """م-24: بطاقةُ النائب الأكاديميّ (1033) بلا بندِ إنابة."""
        ...
        with pytest.raises(PolicyError, match="م-24"):
            DelegationService.grant(school=school, principal=principal_user, delegate=vice_academic)

    def test_an_explicit_delegation_is_still_accepted(self, school, principal_user):
        from staff_affairs.attendance import DelegationService
...
class TestSpecAlignment:
    ...
    def test_a_principal_on_leave_is_replaced(self, school, principal_user):
        """م-25: «غائباً أو في إجازة» — يومُ غيابٍ بنوعه من سجلّ الغياب إجازة."""
        teacher, vice_admin = _staff(school, 1), _actor(school, "vice_admin")
        permit = _permit(school, teacher, FEB, time(7, 0), time(8, 0), "late_arrival", None)
        for role in ("secretary", "vice_academic"):
            PermitService.act(permit, actor=_actor(school, role), approve=True)
    ...
    def test_the_delegation_message_names_the_card_not_a_stale_line(
        self, client_as, school, principal_user
    ):
        """م-25 وز-8: رسالةُ الإنابة لا تنسب إلى «03:401» ما ليس فيه."""
        from django.contrib.messages import get_messages

        vice_academic = _actor(school, "vice_academic")
        client = client_as(principal_user)
        response = client.post(
            reverse("staff_affairs:principal_delegation"), {"delegate": str(vice_academic.pk)}
        )
        texts = [str(m) for m in get_messages(response.wsgi_request)]
        assert texts and all("03:401" not in t for t in texts)
        assert any("م-24" in t for t in texts)
    ...
    def test_the_admin_deputy_decides_form_03_when_the_principal_is_absent(
        self, school, principal_user
    ):
        """م-30 وم-24 (بطاقة 1034: «الإنابة عن المدير في مهامه في حال غيابه») وم-25."""
        from staff_affairs.attendance import ExceptionService
    ...
    def test_the_report_shows_exception_days_apart_from_permits(self, school, principal_user):
        """م-33: لا يُحتسب من سقف 420، ويُعرض في التقرير الشهريّ مستقلّاً."""
        staff = _staff(school, 1)
        _exception(
            school,
            staff,
