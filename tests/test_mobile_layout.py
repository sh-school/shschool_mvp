...
def test_the_schedule_toolbar_does_not_overflow_the_tablet_range():
    ...
    ومنعُ الانكسار من 641px. فالانكسارُ يُمنع من نقطة الحاسوب المعتمدة (1025px) فما فوق، ولا حدَّ 280px إلّا على
    قائمة اختيار الجدول. والحارسُ ساكنٌ لأنّ عرضَي التدقيق (حاسوب وجوّال) لا يلمسان مدى 641–1024.
    """
    one_line = _min_width_where(".schedule-tools", "flex-wrap", "nowrap")
    assert (
        one_line is not None and one_line >= 1025
    ), f"سطرٌ واحدٌ من {one_line}px — أضيقُ من الشريط، فيفيض في اللوحيّ (D-23)"
