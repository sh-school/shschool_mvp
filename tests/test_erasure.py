...
from core.models import (
    AuditLog, ConsentRecord, CustomUser, ErasureRequest,
    ParentStudentLink, StudentEnrollment,
)
from .conftest import (
    ...
    BehaviorInfractionFactory, LibraryBookFactory, BookBorrowingFactory,
)
...
class TestErasureAPI:
    ...
    def test_parent_can_request_erasure(self, api_parent, student_user, school):
        ...
        resp = client.post("/api/v1/erasure/request/", format="json", data={
    ...
    def test_admin_list_requests(self, api_admin, student_user, school):
        ...
        ErasureRequest.objects.create(
            school=school, student=student_user, requested_by=admin,
    ...
    def test_admin_approve_executes_erasure(self, api_admin, student_with_data, school):
        ...
        # Approve
        resp = client.post(f"/api/v1/erasure/requests/{req_id}/approve/", format="json", data={"note": "تمت الموافقة"})
        assert resp.status_code == 200
    ...
    def test_admin_reject_with_reason(self, api_admin, api_parent, student_user, school):
        ...
        resp = client_p.post("/api/v1/erasure/request/", format="json", data={
            "student_id": str(student_user.id),
            "reason": "طلب محو بيانات الطالب من النظام",
        })
        req_id = resp.data["id"]
    ...
    def test_cannot_approve_non_pending(self, api_admin, api_parent, student_user, school):
        ...
        req_id = resp.data["id"]
        # Reject first
        client_a.post(f"/api/v1/erasure/requests/{req_id}/reject/", format="json", data={"note": "مرفوض"})
        # Try approve
...
class TestErasureService:
    ...
    def test_execute_anonymizes_student(self, student_with_data, school):
        ...
        req = ErasureRequest.objects.create(
            school=school, student=student_with_data,
            requested_by=admin, reason="test erasure",
    ...
    def test_execute_preserves_auditlog(self, student_with_data, school):
        ...
        AuditLog.log(
            user=student_with_data, action='login',
            model_name='CustomUser', object_id=str(student_with_data.pk),
            school=school,
        )
        ...
        req = ErasureRequest.objects.create(
            school=school, student=student_with_data,
            requested_by=admin, reason="test", status='approved', reviewed_by=admin,
        )
