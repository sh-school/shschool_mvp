...
#:   `css_kb` 404 → 460، `js_kb` 333 → 400 (لوحة التحكم تحمّل Chart.js)، الخطوطُ
#:   61 → 120، الصورُ 134 → 220. و`blocking_stylesheets` 8 → 10: كلُّ ملفٍّ فوقها
#:   طلبٌ يحجب الرسمَ (ADR-0003 قسّم الملفَّ ثمانيةً؛ فليكن التالي قراراً لا انزلاقاً).
BUDGET = {
    "cls": 0.1,
    "lcp_ms": 2500,
    "inp_ms": 300,
...
#: صفحةُ الدخول لا تُبلِّغ FCP ولا LCP في Chromium أصلاً: بطاقتُها تدخل بأنيميشن
#: `loginCardIn` من `opacity: 0`. جُرّب فقيس: مع الأنيميشن لا يُبلَّغ شيءٌ (لا في
#: 1.4 ثانية)؛ بلا الأنيميشن أو بلا `opacity` في مفتاحه يُبلَّغ FCP وLCP عند 408–420ms.
#: ولا فيها مبدّلٌ يُنقر عليه فلا INP. (تعديلُ الأنيميشن قرارٌ بصريٌّ منفصل.)
