...
def update_procedure_status(request, proc_id):
    ...
    # Notify review committee when submitted for review
    if new_status == "Pending Review":
        year = request.GET.get("year", "2025-2026")
...
def approve_procedure(request, proc_id):
    ...
    # Notify executor about approval/rejection
...
def review_list(request):
    ...
    # Permission check: admin or review committee member
...
def task_update_modal(request, proc_id):
    ...
    is_executor = request.user.is_admin() or request.user == procedure.executor_user
    ...
    if request.method == "POST":
        ...
        # Handle file upload as evidence
...
def review_evaluate_modal(request, proc_id):
    ...
    if request.method == "POST":
        ...
        evidence_req_note = request.POST.get("evidence_request_note", "").strip()
        procedure.evidence_request_note = evidence_req_note
...
def toggle_evidence_request(request, proc_id):
    ...
    # Permission: admin or review committee
    is_admin = request.user.is_admin()
    is_reviewer = QualityCommitteeMember.objects.filter(
        school=school,
        user=request.user,
    ...
    # Notify executor when evidence is requested
