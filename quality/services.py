...
# ═════════════════════════════════════════════════════════════════════
# خدمات تقييم الأداء الوظيفي
# المرجع: 06_attendance_performance_review.md (المادتان 17 و18)
# ═════════════════════════════════════════════════════════════════════


def has_active_sanction(staff_member: CustomUser, academic_year: str) -> bool:
    """
    يتحقق ما إذا كان على الموظف جزاءٌ تأديبيٌّ نشطٌ في السنة الأكاديمية المعطاة.

    المرجع: 06_attendance_performance_review.md §2.1
    - المادة 17: لا يجوز تقييم الموظف بمستوى ممتاز إذا كان عليه جزاء تأديبي
      (بالخصم من الراتب أو الوقف عن العمل لمدة > 5 أيام، أو مجموع > 10 أيام)
    - المادة 18: لا يجوز تقييم بـ ممتاز أو جيد جداً إذا كان عليه جزاء أشد
      (خصم > 10 أيام أو مجموع خمسة عشر يوماً (15) — لا 25؛ صُحِّح من الـPDF
      بتاريخ 2026-09-17: 02- شؤون الموظفين/02- النظام الوظيفي لموظفي المدارس.pdf
      ص12 (المطبوعة 26)، مادة 18)

    الإرجاع:
        True إذا كان على الموظف جزاء نشط
        False خلاف ذلك

    TODO (بند 1.4): يُربط بـ StaffDisciplinaryAction عند بنائها
    النظام الوظيفي (م31): مدير المدرسة له سلطة توقيع:
        1. الإنذار
        2. اللوم
        3. الخصم من الراتب (حتى 15 يوم/سنة، حد أقصى 3 أيام في المرة)

    والمادتان 85 و89 من قانون الموارد البشرية (15/2016) تحدد جزاءات أشد.
    """
    # بند معلق: StaffDisciplinaryAction لم يُبنَ بعد في 1.4
    # هنا نضع منطق الفحص:
    # 1. البحث عن جزاءات في السنة الأكاديمية
    # 2. حساب مجموع أيام الخصم أو الوقف
    # 3. مقارنة بعتبات المادة 17 و18

    # في الوقت الراهن، نرجع False دائماً (لا توجد جزاءات بعد)
    return False


def can_evaluate_as_excellent(staff_member: CustomUser, academic_year: str) -> bool:
    """
    يتحقق ما إذا كان يمكن تقييم الموظف بمستوى "ممتاز".

    المرجع: 06_attendance_performance_review.md المادة 17

    لا يجوز إعطاء "ممتاز" إذا:
    1. كان على الموظف جزاء تأديبي معين (بالخصم > 5 أيام أو مجموع > 10)
    2. انقطع عن العمل > 5 أيام بدون عذر
    3. تُوفّرت له فرصة تدريب ولم يجتزها بنجاح

    الحالي: نفحص الجزاء فقط
    TODO (1.4 و1.5): إضافة فحوصات الغياب والتدريب
    """
    if has_active_sanction(staff_member, academic_year):
        return False
    return True


def can_evaluate_as_very_good_or_higher(staff_member: CustomUser, academic_year: str) -> bool:
    """
    يتحقق ما إذا كان يمكن تقييم الموظف بمستوى "جيد جداً" أو أعلى.

    المرجع: 06_attendance_performance_review.md المادة 18

    لا يجوز إعطاء "جيد جداً" أو "ممتاز" إذا:
    1. كان على الموظف جزاء تأديبي أشد (خصم > 10 أيام أو مجموع 15 يوماً — لا 25،
       صُحِّح من الـPDF بتاريخ 2026-09-17)
    2. انقطع عن العمل > 10 أيام بدون عذر
    3. تُوفّرت له فرصة تدريب وتخلّف عنه دون عذر
