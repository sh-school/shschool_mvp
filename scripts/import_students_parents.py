"""
سكريبت شامل: استيراد الطلاب + أولياء الأمور + الربط
=======================================================
يعالج الحالات التالية:
1. الطالب موجود → تحديث بياناته فقط
2. الطالب غير موجود → إنشاء حساب جديد
3. ولي الأمر موجود (موظف أو غيره) → إضافة دور parent فقط
4. ولي الأمر غير موجود → إنشاء حساب جديد
5. ولي أمر له أكثر من طالب → ربط بجميعهم
...
def run():
    ...
    if not school:
        print("❌ لا توجد مدرسة. أنشئ مدرسة أولاً.")
        return
    ...
    if missing_roles:
        print(f"❌ أدوار مفقودة: {missing_roles}")
        print("   شغّل: python manage.py seed_data  لإنشاء الأدوار")
        return
    ...
    if not os.path.exists(CSV_PATH):
        ...
        print("   انسخ الملف إلى: scripts/new_students_full.csv")
        return
    ...
    print(f"👨‍👩‍👧 أولياء أمر موجودون:         {stats['parents_existed']:>4}")
    print(f"🔗 روابط جديدة أُنشئت:         {stats['links_created']:>4}")
    print(f"🔗 روابط كانت موجودة:          {stats['links_existed']:>4}")
