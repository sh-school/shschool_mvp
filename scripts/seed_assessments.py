...
from core.models import School, CustomUser, ClassGroup, StudentEnrollment, Membership
...
PACKAGES_CONFIG = {
    "S1": [
        ...
        {"type": "P2", "weight": Decimal("0"),     "max": Decimal("40"), "label": "اختبارات قصيرة (غ.م.)"},
        {"type": "P3", "weight": Decimal("0"),     "max": Decimal("40"), "label": "نصفي (غ.م.)"},
        {"type": "P4", "weight": Decimal("50"),    "max": Decimal("40"), "label": "اختبار نهاية الفصل الأول"},
    ],
    "S2": [
        {"type": "P1", "weight": Decimal("16.67"), "max": Decimal("60"), "label": "أعمال مستمرة"},
        {"type": "P2", "weight": Decimal("0"),     "max": Decimal("60"), "label": "اختبارات قصيرة (غ.م.)"},
        {"type": "P3", "weight": Decimal("33.33"), "max": Decimal("60"), "label": "اختبار منتصف الفصل الثاني"},
        {"type": "P4", "weight": Decimal("50"),    "max": Decimal("60"), "label": "اختبار نهاية العام"},
    ],
}
...
def run():
    ...
    default_user  = principal.user if principal else CustomUser.objects.filter(is_superuser=True).first()
    ...
    with transaction.atomic():
        ...
        for name, code in all_subjs:
            s, _ = Subject.objects.get_or_create(school=school, name_ar=name, defaults={"code": code})
            subj_map[name] = s
        ...
        for cg in classes:
            ...
            for name, _ in subj_list:
                ...
                setup, sc = SubjectClassSetup.objects.get_or_create(
                    school=school, subject=subj, class_group=cg, academic_year=year,
                ...
                for sem, pkg_list in PACKAGES_CONFIG.items():
                    for pc in pkg_list:
                        _, pc_new = AssessmentPackage.objects.update_or_create(
                            setup=setup, package_type=pc["type"], semester=sem,
                            defaults={
                                "school":             school,
                                "weight":             pc["weight"],
        ...
        sample = ClassGroup.objects.filter(
            school=school, academic_year=year, grade="G7"
        )[:2]
        ...
        for cg in sample:
            ...
            for setup in setups_c:
                for sem in ("S1", "S2"):
                    pkgs = AssessmentPackage.objects.filter(
                        setup=setup, semester=sem, weight__gt=0
                    )
                    for pkg in pkgs:
                        for asmnt in pkg.assessments.filter(status="published"):
                            for enr in students:
                                ...
                                StudentAssessmentGrade.objects.update_or_create(
                                    assessment=asmnt, student=enr.student,
                                    defaults={
                                        "school":    school,
                                        "grade":     grade,
                                        "is_absent": is_absent,
