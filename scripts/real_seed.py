...
from core.models import School, CustomUser, Role, Membership, ClassGroup, StudentEnrollment, Profile
...
def map_role(job_title_norm: str) -> str:
    ...
    if "مدير المدرسه" in t:                   return "principal"
    if "النائب الاداري" in t:                 return "vice_admin"
    if "نائب المدير للشؤون" in t:             return "vice_academic"
...
def run():
    ...
    with transaction.atomic():
        ...
        for row in staff_rows:
            ...
            Membership.objects.get_or_create(
                user=user, school=school, role=role_obj,
        ...
        for row in student_rows:
            ...
            if cg:
                enr, e_created = StudentEnrollment.objects.get_or_create(
                    student=student, class_group=cg,
                    defaults={"is_active": True}
                )
                if e_created:
                    enr_created += 1
            ...
            if par_nat and par_name:
                ...
                if not parent.password or parent.password == "":
                    parent.set_password("parent@2026")
                    parent.save(update_fields=["password"])

                Profile.objects.get_or_create(
                    user=parent,
                    defaults={"notes": f"علاقة: {par_rel} | ابنه: {name_ar}"}
                )
        ...
        # ═══════════════════════════════════════════════════
        principal_user = school.memberships.filter(role__name="principal").select_related("user").first()
        if principal_user:
    ...
    print(f"  موظفون:              {school.memberships.filter(role__name__in=['principal','vice_admin','vice_academic','coordinator','teacher','specialist','admin']).count()}")
    ...
    print(f"  تسجيلات الطلاب:      {StudentEnrollment.objects.filter(class_group__school=school).count()}")
    print("\n── بيانات الدخول ─────────────────────────────────────")
    if principal_user:
        print(f"  المدير:    {principal_user.user.national_id} / school@2026")
    print(f"  المعلمون:  <الرقم الوطني>    / school@2026")
    print(f"  الطلاب:    <الرقم الوطني>    / student@2026")
    print(f"  الأولياء:  <الرقم الوطني>    / parent@2026")
    print("═"*55)
