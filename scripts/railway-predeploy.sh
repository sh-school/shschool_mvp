...
#
# كان هذا كلُّه داخل railway-release.sh (start) وحده، فمع عدّة نسخٍ
...
# يمسّ الحركةَ الحيّة.
#
# أُكِّد عملُه على نشرٍ حقيقيّ 2026-09-17 (بعد `railway config apply`): سجلّ
# Railway (تبويب «Pre-Deploy» في لوحة النشر — لا تُظهره أداة السطر بخياراتها
# الاعتياديّة) يطبع «✅ Pre-Deploy Phase Complete» ثمّ يُعيد تشغيل الحاوية قبل
# أن يبدأ scripts/railway-release.sh. فحُذف التكرارُ من ذاك الملفّ.
set -e

# تعيين مسار Python صراحةً لضمان إيجادُ المشروع والمودولات المحلّية
# ✅ v5.2: هذا يحلّ مشكلة ModuleNotFoundError أثناء `manage.py migrate`
# عندما تكون ديرِ المشروع الحالي غير محدَّد بوضوح
cd /app
