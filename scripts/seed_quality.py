...
def run():
    ...
    from quality.models import (
        ...
        OperationalProcedure, QualityCommitteeMember,
    )
    ...
    staff_users = CustomUser.objects.filter(
        ...
        memberships__role__name__in=["principal","vice_admin","vice_academic","coordinator","teacher","specialist","admin"],
        is_staff=True
    ).distinct()
    ...
    with transaction.atomic():
        ...
        for i, row in enumerate(plan_rows):
            ...
            if domain_name not in domain_cache:
                domain, _ = OperationalDomain.objects.get_or_create(
                    school=school, name=domain_name, academic_year=year,
            ...
            if t_key not in target_cache:
                target, _ = OperationalTarget.objects.get_or_create(
                    domain=domain, number=target_no,
                    defaults={"text": target_text}
                )
            ...
            if indicator_no not in indicator_cache:
                indicator, _ = OperationalIndicator.objects.get_or_create(
                    target=target, number=indicator_no,
                    defaults={"text": indicator_txt}
                )
            ...
            proc, created = OperationalProcedure.objects.get_or_create(
                ...
                defaults={
                    ...
                    "status":                   status if status in ("In Progress","Completed","Cancelled","Not Started") else "In Progress",
                    "evidence_type":            evidence_type if evidence_type in ("وصفي","كمي","كمي/وصفي") else "",
        ...
        print(f"   مؤشرات:   {OperationalIndicator.objects.filter(target__domain__school=school).count()}")
        print(f"   إجراءات:  {OperationalProcedure.objects.filter(school=school).count()} ({proc_created} جديد، {proc_updated} محدَّث)")

        # إحصائيات الحالات
        from django.db.models import Count
        status_stats = OperationalProcedure.objects.filter(school=school).values("status").annotate(c=Count("id"))
        for s in status_stats:
            label = {"In Progress": "قيد التنفيذ", "Completed": "مكتمل"}.get(s["status"], s["status"])
            print(f"   {label}: {s['c']}")

        # الإجراءات المرتبطة بمستخدم
        linked = OperationalProcedure.objects.filter(school=school, executor_user__isnull=False).count()
        ...
        for d in OperationalDomain.objects.filter(school=school, academic_year=year):
            # تطبيع بسيط
            norm = d.name.replace("ة","ه").replace("ى","ي").replace("أ","ا").replace("إ","ا").lower()
