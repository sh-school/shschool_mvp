...
def run():
    ...
    with transaction.atomic():
        ...
        users_data = [
            ...
            ("00000000001", "مدير تجريبي", "admin123", "principal", True),
            ("00000000002", "نائب إداري تجريبي", "admin123", "vice_admin", False),
            ("00000000003", "نائب أكاديمي تجريبي", "admin123", "vice_academic", False),
            ("00000000004", "معلم تجريبي أول", "teacher123", "teacher", False),
            ("00000000005", "معلم تجريبي ثانٍ", "teacher123", "teacher", False),
            ("00000000006", "معلم تجريبي ثالث", "teacher123", "teacher", False),
            ("00000000007", "منسق تجريبي", "teacher123", "coordinator", False),
            ("00000000008", "معلم تجريبي رابع", "teacher123", "teacher", False),
            ("00000000009", "معلم تجريبي خامس", "teacher123", "teacher", False),
            ("00000000010", "أخصائي تجريبي", "teacher123", "specialist", False),
        ]
        ...
        for nid, name, pwd, role_name, is_staff in users_data:
            ...
            Membership.objects.get_or_create(
                user=u, school=school, role=roles[role_name], defaults={"is_active": True}
            )
            staff[nid] = u
        ...
        for i, name in enumerate(student_names):
            ...
            u, created = CustomUser.objects.get_or_create(
                national_id=nid, defaults={"full_name": name, "is_active": True}
            )
            if created:
                u.set_password(nid[-6:])
                u.save()
            Membership.objects.get_or_create(
                user=u, school=school, role=roles["student"], defaults={"is_active": True}
            )
            StudentEnrollment.objects.get_or_create(
                student=u, class_group=cg_7_1, defaults={"is_active": True}
            )
            students.append(u)
        ...
        for name in subjects_data:
            s, _ = Subject.objects.get_or_create(
                school=school, name_ar=name, defaults={"code": name[:4]}
            )
            subjects[name] = s
