...
| تعريف 5 (نتيجةُ القارئ المُقيَّمة في العرض، والمساعدُ بمرجعه، وكلُّ وحدةٍ غيرِ `selectors`/`services`) — الشيفرةُ نفسُها | 228 | 44 | 224 | 105 | 66 / 16 |
...
## حدودُ الحارس المعروفة

قياسٌ ساكنٌ على الشيفرة، لا حارسٌ يصدّ من يتعمّد الالتفاف عليه. ثلاثةٌ ثبتت بمراجعةٍ
عدائيّة (2026-09-16)، وسُدّت الأولى؛ والباقيتان موثَّقتان لا مُصلَحتان — إصلاحُهما
يحتاج تحليلاً عابراً للملفّات (dataflow) لا شجرة `ast` لملفٍّ واحد:

1. **`--rebaseline --ref` إلى إيداعٍ غيرِ الرأس** كان يتيح إعادةَ القياس على تاريخٍ
   غيرِ ما في الشجرة، فتذوب زيادةٌ حقيقيّةٌ بين تعريفٍ وآخر. **سُدَّت**: يُرفض الآن ما
   لم يكن `--ref` هو الرأسَ نفسه (بمعرّفه الكامل أو حرفَ `HEAD`)، بمقارنة
   `git rev-parse` للاثنين.
2. **الاستيرادُ داخل الدالّة** (`from .helpers import heavy_reader`) يُخفي استعلاماتِ
   الدالّة المستورَدة عن حمل العرض، لأنّ العدّ يتتبّع الاستيرادات على مستوى الملفّ.
   ويُخفي بالمثل QuerySet قارئٍ يُقيَّم داخل العرض نفسه إن جاء اسمُه من استيرادٍ محليّ.
3. **الطرقُ الموروثة** — تابعٌ في صنفٍ أبٍ أو mixin في ملفٍّ آخر تستدعيه الدالّةُ عبر
   نسخةٍ من صنفٍ فرعيّ — لا تُحمَّل على العرض، لأنّ العدّ يتتبّع الاستدعاءَ باسمه لا
   بحلّ الوراثة.

الأصلح لهاتين: مراجعةٌ يدويّةٌ لطلبات الدمج التي تُدخل استيراداً محليّاً جديداً أو
تركّب على mixin تحتوي استعلاماتٍ ثقيلة، لا توسيعُ الحارس — كلُّ سدٍّ آليٍّ لهما يفتح
طريقاً ثالثة.
