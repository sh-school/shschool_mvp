...
| **الاتّجاه والتخطيط** | `test_rtl_logical_properties`، `test_reflow`، `test_z_scale` (ومعه K18 = 0 وشريطُ التثبيت `--z-banner` (300) تحت ما يفتحه المستخدم)، `test_safe_area_tokens`، `test_pwa_banner_position`، `test_mobile_layout`، `test_mobile_nav_menu`، `test_action_cards_fit_narrow_screens`، `test_exec_dash_column` | لا `left/right` مطلقاً؛ لا عرضَ ثابتاً > 320px (WCAG 1.4.10)؛ سلّمُ z-index؛ قواعدُ الجوال؛ نصُّ بطاقة الإجراء لا يفيض عند 375/360/320؛ **غلافُ `.exec-dash` بعمودٍ صريحٍ `minmax(0, 1fr)`** لا `auto` الضمنيّ |
